...
1. [ ] Introduce a PRESUBMIT and SUBMIT script in `//` that are source of truth for their relevant step
2. [ ] Introduce a FIX script in `//` that automatically performs necessary fixes including
   (1) rust deps fix (repinning); (2) version bumping; (3) automatic lint fixes.
3. [ ] Alter the merge workflow to require a green 'SUBMIT' script run. This workflow
