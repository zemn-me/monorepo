...
const htmlDate:
    (d: Date) => string
=
    d => `${htmlMonthString(d)}-${ensurePad(d.getUTCDate().toString(), 2, "0")}`
;
 ...
 */
const ensurePad:
    (s: string, n: number, pad: string) => string
=
    (s, n, p) => p.repeat(n-s.length) + s
;

const htmlYear:
    (d: Date) => string
=
    d => `${ensurePad(d.getUTCFullYear().toString(), 4, "0")}`
;
...
const htmlMonth:
    (d: Date) => string
=
    d => `${ensurePad(d.getUTCMonth().toString(), 2, "0")}`
;

const htmlMonthString:
    (d: Date) => string
=
    d => `${htmlYear(d)}-${htmlMonth(d)}`
;