...
(async function main() {
    ...
    let files = await new Promise((ok, fail) => glob(`./EBGaramond12/fonts/*/*`, {}, (err, files) => {
    ...
    for (const file of files) {
        ...
        m.set(
            key,
            (m.get(key) || []).concat(file)
        );
    }


    const defs = [...m].map(([ name, fonts  ]) => {
        return `
@font-face {
    ...
    src:
        local('${name}'),
        ${
            fonts.map(f => `url(${f}) format('${formatName(path.extname(f).slice(1))}')`).join(",\n")
        };
}
        `
    }).join("\n");
