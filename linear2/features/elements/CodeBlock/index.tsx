...
declare module 'react-syntax-highlighter' {
    ...
    type Properties<T extends keyof JSX.IntrinsicElements = keyof JSX.IntrinsicElements> =
        { className: string [] } & Omit<JSX.IntrinsicElements[T], 'className'>;

    type Element<T extends keyof JSX.IntrinsicElements = keyof JSX.IntrinsicElements>
        = __Element & { tagName: T };

    type __Element = ValueOf<{ [k in keyof JSX.IntrinsicElements]: _Element<k> }>;

    interface _Element<T extends keyof JSX.IntrinsicElements = keyof JSX.IntrinsicElements> {
        type: 'element'
        tagName: T
        children: Node[]
...
const Span:
    (props: highlight.Element<'span'>) => React.ReactElement | null
=
    ({ properties, children }) => {
        const classes = properties.className.map(k => style[k])
                .filter(<T extends any>(v: T | undefined): v is T => v !== undefined);
        
        if (classes.length === 0) return <>{children.map(
            (c, i) => <Node key={i} {...c}/>
        )}</>;
...
const Node:
    (props: highlight.Node) => React.ReactElement | null
=
    props => {
        switch (props.type) {
            ...
            default: throw new Error(`undefined node type ${(props as any).type}`);
        }
    }
;

const Element:
    (props: highlight.Element) => React.ReactElement
=
    props => {
        console.log(props);
...
const renderer: (props: highlight.RendererProps) => React.ReactNode =
    ({ rows }) => <>
            {rows.map((row, i) => <Node key={i} {...row} />)}
    </>

export const CodeBlock: React.FC<CodeProps> =
    ({ children, language, name, lineNumbers = false }) => {

        let o = <SyntaxHighlighter
            style={{}} className={style.code} customStyle={{ backgroundColor: undefined }}
