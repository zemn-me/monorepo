 ...
 */
export const Runs:
    React.FC<{ transform(el: React.ReactNode): React.ReactElement<React.PropsWithChildren<{}>> }>
=
    ({ children, transform }) => {
        ...
        React.Children.forEach(children, child => {
            ...
            if (last?.type == n.type) {
                if (!n.props.children) throw new Error("this edge case is unlikely and i dont care about it");
        ...
        return <>
            {runs.map( (el, i) => <React.Fragment key={i}>{el}</React.Fragment>)}
        </>
    }
;