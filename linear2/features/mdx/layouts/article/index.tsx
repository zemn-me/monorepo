...
const Footnoteref = (props: AProps) => (
	<a
		{...{
			...props,
...
const A: (props: AProps) => React.ReactElement = (props) => {
	switch (props.className) {
		...
		case 'footnote-ref':
			return <Footnoteref {...props} />
	}

	return <a {...props} />
}

const Footnotes: React.FC = ({ children }) => (
	<aside className={style.Footnotes}>{children}</aside>
)
...
const CodeBlock: React.FC<elements.CodeProps & PropsOf<'code'>> = ({
	className,
	...props
}) => {
...
const frac: React.FC = ({ children }) => (
	<div className={style.frac}>{children}</div>
)

const Ss02: React.FC = ({ children }) => (
...
const Chem: React.FC<{ children: string; name?: string }> = ({
	children,
	name,
}) =>
	name ? (
		<abbr className={style.chemical} title={name}>
			<ChemStr>{children}</ChemStr>
		</abbr>
	) : (
		<ChemStr>{children}</ChemStr>
	)
...
const Section = (a: elements.fancy.SectionProps) => (
	<elements.fancy.Section
		{...a}
 ...
 */
const findH1: (
	e: MDXElementChildren,
) => React.ReactElement<MDXElementProps> | undefined = (c) => {
