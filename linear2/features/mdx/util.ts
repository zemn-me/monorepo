...
export interface MDXCreateElementType extends React.ExoticComponent<MDXCreateElementProps> {
    displayName: "MDXCreateElement"
}

export interface MDXCreateElement extends React.ReactElement<
...
export function* visit<T2, T extends { props?: { children?: T2 | T | undefined | T[] } }>(...nodes: T[]): IterableIterator<T | T2> {
...
interface ExtractableChildProps {
    children?: ExtractableChildElement | ExtractableChildElement[] | string | null
}
 ...
 */
export function extractText(r: React.ReactElement<ExtractableChildProps>): string {
    return [...model.iter.filter(visit<string | null, ExtractableChildElement>(r), (el: React.ReactElement<ExtractableChildProps> | string | null): el is string => typeof el === "string")]
        .join("");
}

export function* getMdxElement(r: React.ReactElement<ExtractableChildProps>): IterableIterator<MDXCreateElement> {
 ...
 */
export function getTitle(r: React.ReactElement<ExtractableChildProps>): string | undefined {
    const el = model.iter.filter(getMdxElement(r), e => e.props.mdxType == "h1")
        .next().value;
