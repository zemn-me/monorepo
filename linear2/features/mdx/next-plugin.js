			...
			() => (tree) =>
				require('unist-util-visit-parents')(
					...
					(node, ancestors) => {
						...
						if (i == -1)
							throw new Error('child not parented to parent')
			...
			() => (tree) =>
				require('unist-util-visit-parents')(
					...
					(node, parents) => {
						...
						node = immediateParent.children[myIndex] =
							node.children[0]
			...
			(() => {
				...
				return () => (tree) =>
					require('unist-util-visit-parents')(
						...
						(node, parents) => {
							const rootNode = parents.find(
								(nd) => (nd.type = 'root'),
							)
							if (!rootNode)
								throw new Error(
									'cannot add page title as exported element: no root note found in path to h1',
								)
							rootNode.children.push({
								type: 'export',
								value: `export const Title = ${compiler.stringify(
									node,
								)};`,
							})
						},
