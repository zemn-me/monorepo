...
async function attempt<I extends unknown[], O>(message: string, f: (...i: I) => O, ...p: I[]): Promise<[O, I]> {
    let errors: unknown[] = [];
...
type Tail<T extends unknown[]> =
    T extends [unknown, ...infer Z]? Z: never;
...
const toPathStrings = (...p: readonly Readonly<Path>[]) => p.map(p => Path.format(p));
const resolve = (...p: readonly Readonly<Path>[]) => Path.parse(Path.resolve(...toPathStrings(...p)));
...
export class Article extends Struct<{
    ...
    private static async readFile(path: Readonly<Path>, ...etc: Tail<Parameters<typeof Article.pReadFile>>) {
        return Article.pReadFile(Path.format(path), ...etc)
    }

    get dirPath() { return resolve(Path.parse(process.cwd()), this.value.dirPath!)}
    ...
    private static async filePathAndContent(target: Readonly<Path>) {
        ...
        for (const ext of extensions) {
            for (const indPath of indexPaths) {
                tries.push([withExt(ext, resolve(target, indPath)), ...tailParams]);
            }
        }
    ...
    get filePath() {
        return this.value.filePath === undefined
            ? (async () => {
                const [ file, [ path ] ] = await Article.filePathAndContent(this.dirPath!);
    ...
    get vfile() {
        return this.value.vfile === undefined
            ?(async () => {
                const [ path, contents ] = [ await this.filePath, await this.content ];
            ...
            ?(async() => {
                ...
                visit<mdast.Heading & { depth: 1 }>(
                    ...
                    (node: unknown): node is mdast.Heading & { depth: 1 } =>
                        (node as unist.Node).type == "heading" && (node as unist.Node).depth == 1,
