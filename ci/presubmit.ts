...
const cmd = new Command('presubmit')
	...
	.action(async o => {
		...
		if (!o.skipBazelTests) {
			...
			await new Promise<void>((ok, error) => child_process.spawn('go', ['mod', 'tidy'], {
				cwd,
				stdio: 'inherit',
			}).on('close', code => code == 1? ok(): error(new Error(
				`Go mod tidy exited with ${code}. This likely means that it needs to be run to add / remove deps.`
				))));


			await new Promise<void>((ok, error) => child_process.spawn('bazel', ['run', '//:gazelle-update-repos', '-diff'], {
				cwd,
				stdio: 'inherit',
			}).on('close', code => code == 1? ok(): error(new Error(
			...
			await new Promise<void>((ok, error) => child_process.spawn('bazel', ['run', '//:gazelle', '-diff'], {
				cwd,
				stdio: 'inherit',
			}).on('close', code => code == 1? ok(): error(new Error(
			...
			await new Promise<void>((ok, error) => child_process.spawn('bazel', ['test', '//...'], {
				cwd,
				stdio: 'inherit',
			}).on('close', code => code == 0? ok(): error(new Error(`Exit code: ${code}`))));
