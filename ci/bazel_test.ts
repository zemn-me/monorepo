...
::notice ::  INFO: Build completed, 1 test FAILED, 8634 total actions
::notice title=//%3Avalidate_renovate_config_test passed.,file=/BUILD.bazel:://:validate_renovate_config_test passed.%0A  //:validate_renovate_config_test                                         PASSED in 5.1s
::notice title=//.github%3Avalidation passed.,file=.github/BUILD.bazel:://.github:validation passed.%0A  //.github:validation                                                     PASSED in 0.5s
::warning title=//.github/workflows%3Avalidation failed to build.,file=.github/workflows/BUILD.bazel:://.github/workflows:validation failed to build%0A  //.github/workflows:validation                                           NO STATUS in 1.0s
::error title=//bin/host/ffmpeg%3Asmoke failed.,file=bin/host/ffmpeg/BUILD.bazel:://bin/host/ffmpeg:smoke failed.%0A  //bin/host/ffmpeg:smoke                                                  FAILED in 0.1s`);
});

const later = <T>(): [(value: T | PromiseLike<T>) => void, Promise<T>] => {
	let okay: (value: T | PromiseLike<T>) => void;
	const p = new Promise<T>((ok, error) => (okay = ok));
