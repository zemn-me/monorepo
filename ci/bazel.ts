...
function mergeStreams(stream1: Readable, stream2: Readable): Readable {
...
export async function Bazel(cwd: string, ...args: string[]) {
	...
	for await (const line of AnnotateBazelLines(
		byLine(mergeStreams(process.stdout, process.stderr))
	)) {
		console.log(line);
	}
