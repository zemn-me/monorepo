...
export interface RegistrationContextType {
	register(): [onChange: (record: Record) => void, unregister: () => void]
}
...
export const SectionsProvider: React.FC = ({ children: reactChildren }) => {
	const [children, setChildren] = React.useState<Map<{}, Record>>(
		() => new Map(),
	)
	...
	if (sections[0] && sections[0].children?.length)
		sections[0].children = sections[0].children.filter(
			(v: any) => v.type == 'record',
		)
	...
	const register = React.useCallback<
		RegistrationContextType['register']
	>(() => {
		const key = {}
...
export const SectionManager: React.FC<SectionManagerProps> = ({
	...
	const onVisibilityChanged = React.useCallback(
		([{ isIntersecting: visible }]) => setVisible(() => visible),
		[setVisible],
	)
	...
	const ourRegister = React.useCallback<
		RegistrationContextType['register']
	>(() => {
		const key = {}
	...
	const ctxValue = React.useMemo(() => ({ register: ourRegister }), [
		ourRegister,
	])

	return (
		<RegistrationContext.Provider value={ctxValue}>
			{React.cloneElement(
				childElement,
				{
					...childElement.props,
