...
export class Ctx {
	...
	private handleChange() {
		this.sects = [...this.nodes.values()].sort(
			({ node: a }, { node: b }) => {
		...
		for (let sect of this.sects) {
			while (
				!(
					cur?.self?.level == undefined ||
					cur?.self?.level < sect.level
				)
			) {
			...
			if (cur.parent !== undefined && sect.level == cur?.self?.level)
				cur = cur.parent
