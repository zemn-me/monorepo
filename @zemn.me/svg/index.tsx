...
import * as vector from '@zemn.me/math/vec';
...
export type Path<L extends number = number, kind extends Cartesian | Homogenous = Cartesian> =
    vector.Vector<L, kind>;

export type Cartesian = vector.Vector<2, number>;
export type Homogenous = vector.Vector<3, number>;

export const cart2Homog:
    <L extends number>(path: Path<L, Cartesian>) => Path<L, Homogenous>
=
    path => vector.map(path, ([x, y]) => [x, y, 1] as const)
;

export const homog2Cart:
    <L extends number>(path: Path<L, Homogenous>) => Path<L, Cartesian>
=
    path => vector.map(path, ([x, y, w]) => [x/w, y/w] as const)
;
