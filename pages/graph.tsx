...
const ScalePip: (props: ScalePipProps) => React.ReactElement = ({
	pipLength = 5,
	label,
	className,
	orientation,
...
interface ScaleBandProps<T extends { toString(): string }>
	extends PropsOf<'svg'> {
...
type ScaleProps<T extends { toString(): string }> =
	| ScaleContinuousProps
	| ScaleBandProps<T>
	| ScaleTimeProps

const Scale: <T extends { toString(): string }>(
	props: ScaleProps<T>,
) => React.ReactElement<PropsOf<'svg'>> = (p) => {
	const {
		d3Scale: scale,
		labels,
		orientation,
