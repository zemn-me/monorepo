...
Let's break down what these parameters mean:
 ...
 - `response_type=code` is an OAuth specific parameter asking for an [OAuth code] -- in essence, a ticket that is exchanged by the iCloud backend for an OAuth token which gives the actual access[^3] -- which the iCloud servers can exchange for an Apple ID credential.
...
[refresh token]: https://tools.ietf.org/html/rfc6749#section-6
 ...
 - `redirect_uri` is also an OAuth specific concept, and it kind of exposes the weirdness going on with embedded OAuth.<br/><br/> Typically, once the OAuth dance is complete and the user has granted access, the `redirect_uri` indicates[^4] where the credentials should be sent to, via a full page *redirect*.<br/><br/> You might notice this doesn't happen in apple's embedded iCloud flow, but this parameter still plays something of a role in specifying who the embedding page is (in this case, iCloud).
...
[^4]:
 ...
 - `locale` just indicates the language to show the login dialog in.
 
 - `frame_id` I think is just to help differentiate cases where there may be multiple different embedded pages.
