...
<svg style={{ width: "100%", height: "5rem", background: "white" }}>
    <EventSequence>
        {{ date: SimpleDate.Parse([21, "nov", 2019]), event: "reported" }}
        {{ date: SimpleDate.Parse([29, "jul", 2020]), event: "fixed" }}
    </EventSequence>
</svg>

Reporting this bug was a frustrating experience. But, can I really complain about that? These days, it always is. In short: I reported two bugs in late 2019, both of which were fixed within less than a week. The second bug, the Apple ID code injection took 18 weeks to get accepted and 36 weeks to get paid for. The other, as far as I can tell has never been responded to by a human. I have not been publicly credited for either bug.
