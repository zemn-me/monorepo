...
type props = (ReturnType<typeof getStaticProps> extends Promise<infer Q>
	? Q
	: never)['props']
export default function Dir(props: props) {
	...
	for (const article of props.articles) {
		years.set(
			article.params.path[0],
			(years.get(article.params.path[0]) ?? []).concat(article),
		)
	}
							...
							<ol>
								{articles.map((a: any, i: any) => {
									return (
										<li key={i}>
											<Link
												href={`/article/${a.params.path?.join(
													'/',
												)}`}>
												<a>
													<Render node={a.title} />
