...
level of the website the injection was made in, allowing access to user sessions
and details; anything the user can access or manipulate the attacker can.

With the popularity of content-unaware templating systems such as PHP's built in
system, security often becomes clumsy to implement, where a programmer would
...
Who is the browser working for? {#who_is_the_browser_working_for}
-------------------------------
...
`report-uri` directive for specifying a place to send breaches of the policy to.
...
Detecting a login {#detecting_a_login}
-----------------


Interest piqued by the `report-uri` feature, I looked into abusing it to glean
information about user state, my idea was this: when a user is *not* logged into
Google Calendar, accessing [calendar.google.com] redirects them to
...
within my web page would break CSP, subsequently sending me a message telling me
whether they were logged into Google.

[calendar.google.com]: //calendar.google.com "Google Calendar"
[accounts.google.com]: //calendar.google.com "Google Accounts"
...
`apps.facebook.com` would redirect to `www.facebook.com` *only* if the user was
logged in.
...
Communing with the Spirits {#communing_with_the_spirits}
--------------------------
...
Detecting who is Logged in {#detecting_who_is_logged_in}
--------------------------
...
noticed that though domains were accepted, paths were *also* accepted, which
...
Detecting Barack Obama {#detecting_barack_obama}
----------------------
...
reasonably expect to have logins. By using CSP to whitelist <facebook.com/me>
and <facebook.com/barackobama> and embedding ‘http://facebook.com/me’ as an
...
*who* you were logged in as on those websites.
