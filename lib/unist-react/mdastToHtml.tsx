...
export const linkReference: React.FC<mdast.LinkReference & mdast.Link> = (p) =>
	React.createElement(link, p)

export const heading: React.FC<mdast.Heading> = ({ depth, ...etc }) => (
	<Render node={{ ...etc, type: `h${depth}` }} />
)

export const emphasis: React.FC<mdast.Emphasis> = (nd) => (
	<Render node={{ ...nd, type: 'em' }} />
)

export const inlineCode: React.FC<mdast.InlineCode> = (nd) => (
	<Render
		node={
			{
				...nd,
				type: 'code',
				children: [{ type: 'text', value: nd.value }],
			} as UnifiedNode
		}
	/>
)

export const link: React.FC<mdast.Link> = ({ url: href, ...p }) => (
	<Render node={{ ...p, type: 'a', href }} />
)

export interface Footnotes
	extends unist.Node,
		unist.Parent,
		Omit<PropsOf<'aside'>, 'children'> {
	type: 'footnotes'
}

// this node was sneakily introduced by remark-footnotes. it's not
// to spec.
export const footnotes: (props: Footnotes) => React.ReactElement = (nd) => {
...
export const footnoteReference: React.FC<FootnoteReference> = ({
	identifier,
	label,
	...etc
}) => (
