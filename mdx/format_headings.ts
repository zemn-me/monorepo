...
function getAt(lines: string[], index: number): string {
	return lines[index] ?? '';
}
...
function formatHeadings(text: string): string {
	...
	lines: for (let index = 0; index < lines.length; index++) {
		...
		if (!inFence) {
			...
			if (atxHeading) {
				const marker = getAt(atxHeading, 1) === '#' ? '=' : '-';
				output.push(formatTitle(getAt(atxHeading, 2)));
			...
			if (line !== '') {
				for (const marker of ['=', '-'] as const) {
					if (isSetextUnderline(getAt(lines, index + 1), marker)) {
					...
					if (
						getAt(lines, index + 1) === '' &&
						isSetextUnderline(getAt(lines, index + 2), marker)
					) {
						output.push(formatTitle(line));
						output.push(underlineFor(marker));
						index += 2;
