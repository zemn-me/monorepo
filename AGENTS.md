...
- go/gazelle/ts picks up `@jest-environment jsdom` comments and emits `jsdom = True` on generated `jest_test` rules.
- go/gazelle/ts now generates a dedicated `<name>_tests` ts_project and keeps a private link back to `:<name>` so the deps stay intact during resolve.
