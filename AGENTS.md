...
- Projects (code under `project/`) should not import each other unless the logic is semantically specific to that project (e.g. shared `zemn.me` API client code can live in `project/zemn.me`); broadly reusable logic like 3D rendering should live in `ts/...` (or another language-appropriate shared root).
