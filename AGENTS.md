...
Exception: generated artifacts may be checked in when regenerating them costs
money, such as benchmark results that require paid model/API calls.
...
- First use can stay co-located with the feature that needs it. On the second
  unrelated use, move the shared behaviour into a semantic language-level path
  such as `ts/...`, `go/...`, etc., and make it generic enough for both callers
  (e.g. a second TypeScript OpenAI API caller should promote a local
  `openai.ts` into a shared TS library).
