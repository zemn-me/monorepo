...
async function judgeOpenAI() {
	...
	if (hasFlag('require-complete')) {
		validateCompleteAnswerSet(answers, 'judge-openai --answers');
	}
	const judgeModel = flagValue('judge-model') ?? DEFAULT_OPENAI_JUDGE_MODEL;
	const judgeEfforts = parseReasoningEfforts(
		flagValue('judge-effort') ?? DEFAULT_OPENAI_JUDGE_EFFORT
...
function usage(): string {
	return [
		...
		'Use judge-openai --require-complete for human controls or any baseline that must cover every Mandarin Bench question before paid judging starts.',
	].join('\n');
}
...
async function main() {
	...
	if (command === 'judge-prompts') {
		...
		if (hasFlag('require-complete')) {
			validateCompleteAnswerSet(answers, 'judge-prompts --answers');
		}
