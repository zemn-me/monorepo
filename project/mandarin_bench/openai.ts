...
export const DEFAULT_REASONING_EFFORTS = ['medium'] as const;
...
export function parseReasoningEfforts(raw?: string): ReasoningEffort[] {
	...
	const allowed = new Set<string>(SUPPORTED_REASONING_EFFORTS);
		...
		if (!allowed.has(effort)) {
			throw new Error(
				`Unknown reasoning effort ${JSON.stringify(
					effort
				)}. Use one of: ${SUPPORTED_REASONING_EFFORTS.join(', ')}`
			);
		}
