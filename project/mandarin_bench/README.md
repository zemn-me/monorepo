...
Human-authored controls are kept separate from model runs and must cover the
full fourteen-question benchmark before they are treated as a baseline in the
article chart. A partial historical excerpt can be useful source material, but
it is not a control.

When a complete human answer set is available, write it as JSONL under
`runs/human_controls.jsonl`, with one successful row per benchmark question and
`"control": true` on each row. Then judge it with a completeness guard:

```sh
bazel run //project/mandarin_bench:cli -- judge-openai \
  --answers=$(pwd)/project/mandarin_bench/runs/human_controls.jsonl \
  --judge-model=gpt-5.5 \
  --judge-effort=medium \
  --concurrency=1 \
  --require-complete \
  --resume \
  --out=$(pwd)/project/mandarin_bench/runs/human_control_judgements_gpt_5_5_medium.jsonl
```

The control files are:

- `runs/human_controls.jsonl`: human-authored source answers, one row per
  benchmark question, marked with `"control": true`
