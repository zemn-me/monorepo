...
export function validateCompleteAnswerSet(
	...
	for (const answer of answers) {
		if (!expected.has(answer.question_id)) {
			throw new Error(
				`${label} contains unknown Mandarin Bench question: ${answer.question_id}`
			);
		}
		if (seen.has(answer.question_id)) {
			throw new Error(
				`${label} contains duplicate answer for ${answer.question_id}`
			);
		}
		seen.add(answer.question_id);
		if (answer.answer.trim().length === 0) {
			throw new Error(`${label} contains an empty answer for ${answer.question_id}`);
		}
	}
	...
	if (missing.length > 0) {
		throw new Error(
			`${label} is incomplete: ${seen.size}/${expectedIds.length} questions answered; missing ${missing.join(', ')}`
		);
	}
}
