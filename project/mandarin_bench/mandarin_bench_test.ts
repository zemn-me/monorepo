...
test('complete answer validation rejects partial human controls', () => {
	expect(() =>
		validateCompleteAnswerSet(
			[
				{
	...
	).toThrow('human control is incomplete: 1/14 questions answered');

	expect(() =>
		validateCompleteAnswerSet(
			MANDARIN_BENCH.questions.map(question => ({
				answer: `臣对：${question.title}`,
				control: true,
...
test('OpenAI reasoning defaults to medium and supports explicit efforts', () => {
	...
	expect(DEFAULT_REASONING_EFFORTS).toEqual(['medium']);
