import { useGetExactContactByEmail } from '#root/project/me/zemn/hook/useGetExactContactByEmail.js';
import { displayPersonName } from '#root/ts/google/people/display.js';
import {
	None,
	option_and_then_flatten,
	option_from_maybe_undefined,
	option_unwrap_or,
...
export function usePosterDisplayName(
	...
	const contactName = option_unwrap_or(
		contact(
			() => None,
			person =>
				option_and_then_flatten(person, person =>
	...
	const posterName = [poster?.given_name, poster?.family_name]
		.filter(part => part && part.trim() !== '')
		.join(' ')
		.trim();

	const fallback =
		contactName || posterName || poster?.email_address || poster?.sub || '';
