...
import { H1 } from '#root/project/me/zemn/components/Article/heading.js';
...
const naturals = 'C D E F G A B'.split(' ');

const noteName = Enum(['C', 'D', 'E', 'F', 'G', 'A', 'B']);
...
const midiToNoteName = (n: number): NoteName[] =>
	n % 2 == 0
		? // is natural
			[
				[
					naturals[(n % octave) / tone]! as NoteName2,
					'♮',
					Math.floor(n / octave) - 1,
				], // yes, really
			]
		: // is ♭ / ♯
			[
				// ♯
				[
					naturals[Math.floor((n % octave) / tone)]! as NoteName2,
					'♯',
					Math.floor(n / octave) - 1,
				],
				...
				// ♭
				[
					naturals[Math.ceil((n % octave) / tone)]! as NoteName2,
					'♭',
					Math.floor(n / octave) - 1,
				],
			];

function NoteNameElement({
			...
			<tbody>
				{Array(127)
					...
					.map((_, n) => (
						<tr key={n}>
							<td>{n}</td>
							{midiToNoteName(n).map((nn, n2, a) => (
								<td colSpan={3 - a.length} key={n2}>
		...
		<Article>
			<>
				...
				<Section>
					...
					<p>
						The following are notes from my studying the book "A
						geometry of music" via Strudel.
					</p>
					<p>Zero midi is C (but like a crazy low c).</p>
					<Strudel code={`$: note("0 c").sound("piano")`} />
					<p>
						Midi number increments correspond to one semitone (half
						a note).
					</p>
					<p>
						Therefore, every even midi note is a "whole note" (A, B
						C).
					</p>
					...
					<Strudel
						code={`$: note("c4 60") // these are the same note`}
					/>
					<Strudel code={`$: note("50 52").sound("piano") // D, E`} />
					<blockquote>
						Melodies tend to move by short distances from note to
						note.
					</blockquote>
					<p>
