...
type Measurement<
	...
	U extends Unit = Unit,
> = measurement<[scalar: S, unit: U]>;

function mustUnit(unit: string): Result.Result<Unit, Error> {
	if (!Object.keys(measurementConversions).some(v => v == unit))
		return Result.Err(new Error(`Unknown unit: ${unit}`));

	return Result.Ok(unit as Unit);
}
...
class measurement<T> extends NewType<T> {
	...
	as_m(this: Measurement): measurement<[number, 'm']> {
		return new measurement([
			this.scalar() * measurementConversions[this.unit()],
			'm',
		]);
	}
	...
	as(this: Measurement, target: Unit): Measurement {
		return new measurement([
			this.as_m().scalar() / measurementConversions[target],
			target,
		]);
	}
	...
	plus(this: Measurement, other: Measurement): Measurement<number, 'm'> {
		return new measurement([
			this.as_m().scalar() + other.as_m().scalar(),
			'm',
		]);
	}
	...
	multiply_unitless(
		this: Measurement<number, 'in'>,
		other: number
	): Measurement<number, 'in'>;
	multiply_unitless(
		this: Measurement<number, 'cm'>,
		other: number
	): Measurement<number, 'cm'>;
	multiply_unitless(
		this: Measurement<number, 'mm'>,
		other: number
	): Measurement<number, 'mm'>;
	multiply_unitless(
		this: Measurement<number, 'm'>,
		other: number
...
class ParseMeasurementError extends Error {
	constructor(cause: Error, input: string, meaning: string) {
		const inputRepresentation =
			input == '' ? `an empty ${meaning}` : `${meaning} "${input}"`;
...
function parseMeasurement(
	...
	if (unit == undefined)
		return Result.Err(
			new ParseMeasurementError(
				new Error('please specify a unit'),
				measurement,
				meaning
			)
		);
	...
	if (isNaN(scalar))
		return Result.Err(
			new ParseMeasurementError(
				new Error('please specify a valid number'),
				measurement,
				meaning
			)
		);

	return Result.and_then(mustUnit(unit), unit => Measurement([scalar, unit]));
}

function displayCanonicalUnit(
	measurement: Result.Result<Measurement, unknown>
): string {
	return Result.unwrap_or(
		Result.and_then(measurement, value => value.to_string()),
		'unknown'
	);
}

const normalisedFrameSizes = frameSizes
	.map(v => ({
		...v,
		width: Result.unwrap(parseMeasurement(v.width, `${v.name} width`)),
		height: Result.unwrap(parseMeasurement(v.height, `${v.name} height`)),
	}))
	.map(v => [
		v,
		{
...
function VisualiseFramedArt({
	...
	return (
		<svg
			...
			viewBox={[0, 0, framew, frameh].join(' ')}
		>
			<rect
				height={arth}
...
function matteboardCuts({
	frameHeight,
	overlapAmount,
	artHeight,
	frameWidth,
	artWidth,
}: MatteboardCutsProps): MatteBoardCutsOutputs {
	...
	const windowHeight = frameHeight.minus(
		matteboardInsetHeight.multiply_unitless(2)
	);

	const windowWidth = frameWidth.minus(
		matteboardInsetWidth.multiply_unitless(2)
	);

	return {
		backingInsetHeight,
		backingInsetWidth,
		matteboardInsetHeight,
		matteboardInsetWidth,
		windowHeight,
		windowWidth,
	};
}
...
export function FrameClient() {
	...
	const frameWidth = useMemo(
		() => parseMeasurement(frameWidthInput, 'frame width'),
		[frameWidthInput]
	);
	...
	const frameHeightChange = useCallback(
		(e: ChangeEvent<HTMLInputElement>) =>
			void setFrameHeightInput(e.target.value),
		[setFrameHeightInput]
	);
	const frameHeight = useMemo(
		() => parseMeasurement(frameHeightInput, 'frame height'),
		[frameHeightInput]
	);
	...
	const artWidthChange = useCallback(
		(e: ChangeEvent<HTMLInputElement>) =>
			void setArtWidthInput(e.target.value),
		[setArtWidthInput]
	);
	const artWidth = useMemo(
		() => parseMeasurement(artWidthInput, 'art width'),
		[artWidthInput]
	);
	...
	const artHeightChange = useCallback(
		(e: ChangeEvent<HTMLInputElement>) =>
			void setArtHeightInput(e.target.value),
		[setArtHeightInput]
	);
	const artHeight = useMemo(
		() => parseMeasurement(artHeightInput, 'art height'),
		[artHeightInput]
	);
	...
	const overlapAmountChange = useCallback(
		(e: ChangeEvent<HTMLInputElement>) =>
			void setOverlapAmountInput(e.target.value),
		[setOverlapAmountInput]
	);
	const overlapAmount = useMemo(
		() => parseMeasurement(overlapAmountInput, 'overlap amount'),
		[overlapAmountInput]
	);
	...
	const minimumCutDepthInputChange = useCallback(
		(e: ChangeEvent<HTMLInputElement>) =>
			void setMinimumCutDepthInput(e.target.value),
		[setMinimumCutDepthInput]
	);
	const minimumCutDepth = useMemo(
		() =>
			Result.unwrap_or(
				parseMeasurement(minimumCutDepthInput, 'minimum cut depth'),
	...
	const onFrameSwapClick = useCallback(
	...
	const onArtSwapClick = useCallback(
		(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
			e.preventDefault();
	...
	const artInfo = Result.zip(artHeight, artWidth);
	const frameInfo = Result.zip(frameHeight, frameWidth);
	const errataInfo = Result.zipped(
		overlapAmount,
		Result.Ok(minimumCutDepth),
		(overlapAmount, minimumCutDepth) =>
			[overlapAmount, minimumCutDepth] as const
	);
	const frameOptionsInfo = Result.zip(artInfo, errataInfo);

	const frameOptions: JSX.Element = Result.unwrap_or_else(
		Result.and_then(
			Result.and_then(
				frameOptionsInfo,
				([[artHeight, artWidth], [overlapAmount, minCutDepth]]) =>
					Iterable(normalisedFrameSizes)
						...
						.map(v => {
							if (v.δheight.scalar() < 0 || v.δwidth.scalar() < 0)
								return Option.None;
							...
							const vn = {
								...
								mattedHeight,
								mattedWidth,
								totalMattedArea: mattedWidth.plus(mattedHeight),
								...matteboardCuts({
							...
							if (
								vn.matteboardInsetHeight.less_than(
									minCutDepth
								) ||
								vn.matteboardInsetWidth.less_than(minCutDepth)
							)
								return Option.None;

							return Option.Some(vn);
						})
						.filter()
						.sort((a, b) =>
							a.totalMattedArea.minus(b.totalMattedArea).scalar()
						)
						.map(v => (
							<tr key={v.name}>
								<td>{v.name}</td>
								...
								<td>
									<button
										aria-label="pick this frame"
										onClick={e => {
											void setFrameWidthInput(
												v.width.to_string_no_adjustments()
											);
											void setFrameHeightInput(
												v.height.to_string_no_adjustments()
											);
											e.preventDefault();
										}}
									>
			...
			values => {
				...
				if (rows.length < 1) return <>No results.</>;
	...
	const cuts = Result.and_then_flatten(
		Result.zip(artInfo, frameInfo),
		([[artHeight, artWidth], [frameHeight, frameWidth]]) =>
			Result.and_then(overlapAmount, overlapAmount =>
				matteboardCuts({
					frameHeight,
					frameWidth,
					artHeight,
	...
	return (
		<form>
			...
			<section
				style={{ top: '0', backgroundColor: 'var(--background-color)' }}
			>
				<fieldset>
					<legend>Frame</legend>
					...
					<button
						id={frameSwapClickButtonId}
						onClick={onFrameSwapClick}
				...
				<fieldset>
					...
					<label htmlFor={artHeightInputId}>
						...
						<input
							...
							pattern={reParseMeasurement.source}
							value={artHeightInput}
						/>
					</label>
					<button
						id={artSwapClickButtonId}
						onClick={onArtSwapClick}
						title="swap width and height"
					>
						⇄
					</button>
			...
			<output
				htmlFor={[
					...
					artWidthInputId,
					artHeightInputId,
					overlapAmountInputId,
					artSwapClickButtonId,
					frameSwapClickButtonId,
				].join(' ')}
				name="result"
			>
				<h2>Visualisation.</h2>
				...
				<details>
					...
					<p>
						To help you pick what sized frame to put the art in,
						this is a list of different standard & off the shelf
						frame sizes sorted by least space around the art to
						most.
					</p>
					{frameOptions}
				</details>
				...
				<dl>
					<dt>
						inset height:{' '}
						{displayCanonicalUnit(
							Result.and_then(cuts, v => v.backingInsetHeight)
						)}
					</dt>
					<dd>
						This is the depth of the line to draw on the foam core
						from the bottom and top when it is in portrait to
						accurately place the art in the centre.
					</dd>
					<dt>
						inset width:{' '}
						{displayCanonicalUnit(
							Result.and_then(cuts, v => v.backingInsetWidth)
						)}
					</dt>
					<dd>
						This is the depth of the line to draw on the foam core
						from the bottom and top when it is in <i>landscape</i>{' '}
						to accurately place the art in the centre.
					</dd>
					<dt>
						matte inset height:{' '}
						{displayCanonicalUnit(
							Result.and_then(cuts, v => v.matteboardInsetHeight)
						)}
					</dt>
					<dd>
						This is the depth of the line to draw on the back of the
						matteboard from the bottom and top when it is in
						portrait.
					</dd>
					<dt>
						matte inset width:{' '}
						{displayCanonicalUnit(
							Result.and_then(cuts, v => v.matteboardInsetWidth)
						)}
					</dt>
					<dd>
						This is the depth of the line to draw on the back of the
				...
				<p>
					You will need a matteboard cutter with a regular blade and a
					bevel blade, as well as acid-free tape, matteboard and foam
					core.
				</p>
				<ol>
					<li>
						Cut the matteboard and foam core to the same size as the
						inner size of your frame.
					</li>
					<li>
						Set the cut depth of the matteboard cutter to 'inset
						height'.
					</li>
					<li>
						Place the foam core in the matteboard cutter in portrait
						and scribe a line against the cutter runner in pencil.
					</li>
					<li>Rotate the foam core 180°.</li>
					<li>Scribe a line against the cutter runner in pencil.</li>
					<li>
						Repeat steps 3-5 for the <i>back</i> of the matteboard.
					</li>
					<li>
						Set the cut depth of the matteboard cutter to 'inset
						width'.
					...
					<li>
						Place the foam core in the matteboard cutter in
						landscape and scribe a line against the cutter runner in
						pencil.
					</li>
					<li>Rotate the foam core 180°.</li>
					<li>Scribe a line against the cutter runner in pencil.</li>
					<li>
						Repeat the last 3 steps for the back of the matteboard.
					</li>
					<li>
						(you now have both the matteboard and the foam core
						inscribed with a box the size of your art)
					</li>
					<li>
						Set the cut depth of the matteboard cutter to 'matte
						inset height', and repeat the process – except only on
						the back of the matteboard.
					</li>
					<li>Do the same for 'matte inset width'.</li>
					<li>
						(you now have a smaller box indicating the size of the
						image that will show through the hole in the matteboard)
					</li>
