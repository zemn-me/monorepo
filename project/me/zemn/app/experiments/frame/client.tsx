...
function parseMeasurement(
	measurement: string,
	meaning: string
): Result<Measurement, ParseMeasurementError | Error> {
	...
	return mustUnit(unit).and_then(unit => Measurement([scalar, unit]));
}

function displayCanonicalUnit(
	measurement: Result<Measurement, unknown>
): string {
...
export function FrameClient() {
	...
	const minimumCutDepth = useMemo(
		() =>
			parseMeasurement(
				...
				'minimum cut depth'
			).unwrap_or(Measurement<[number, 'cm']>([0, 'cm'])),
		[minimumCutDepthInput]
	);
		...
		.and_then(([[artHeight, artWidth], [overlapAmount, minCutDepth]]) =>
			Iterable(normalisedFrameSizes)
				...
				.map(v => {
					...
					if (
						vn.matteboardInsetHeight.less_than(minCutDepth) ||
	...
	const cuts = artInfo
		.zip(frameInfo)
		.and_then(([[artHeight, artWidth], [frameHeight, frameWidth]]) =>
	...
	return (
		<form>
			...
			>
				...
				{artInfo
					.zip(frameInfo)
					.and_then(([[artW, artH], [frameW, frameH]]) => (
						<VisualiseFramedArt
				...
				<dl>
					<dt>
						...
						{displayCanonicalUnit(
							cuts.and_then(v => v.backingInsetHeight)
						)}
					</dt>
					<dd>
					...
					<dt>
						inset width:{' '}
						{displayCanonicalUnit(
							cuts.and_then(v => v.backingInsetWidth)
						)}
					</dt>
					<dd>
						This is the depth of the line to draw on the foam core
					...
					<dt>
						matte inset height:{' '}
						{displayCanonicalUnit(
							cuts.and_then(v => v.matteboardInsetHeight)
						)}
					</dt>
					<dd>
					...
					<dt>
						matte inset width:{' '}
						{displayCanonicalUnit(
							cuts.and_then(v => v.matteboardInsetWidth)
						)}
					</dt>
					<dd>
						This is the depth of the line to draw on the back of the
