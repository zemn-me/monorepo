...
const columnsInStringRow = (s: string) =>
	[...s].map(v => (v === '\t' ? 4 : 1)).reduce((a, b) => a + b, 0);
...
function stretchDots(s: string) {
	...
	return s.replace(
		/(?<= |^)\.+ +/g,
		match => match.slice(0, -1).replaceAll(/./g, '.') + ' '
	);
}
...
function ElasticTabStopsClientForm() {
	...
	const [output, [outputRows, outputColumns], [inputRows, inputColumns]] =
		useMemo(() => {
			let v = input;
			...
			return [output, outputRowsColumns, inputRowsColumns];
	...
	const onExampleButtonClick = useCallback(
	...
	return (
		<>
			...
			<p>
				...
				<Link href="https://nick-gravgaard.com/elastic-tabstops/">
					Elastic Tabstops algorithm
				</Link>{' '}
				via a machine transation of the{' '}
				<Link href="https://pkg.go.dev/text/tabwriter">
					Go implementation (OpenAI o3)
				</Link>{' '}
				to TypeScript.
			</p>

			<p>
				Click <button onClick={onExampleButtonClick}>here</button> for
				an example.
			</p>
			<form>
				<label
					htmlFor={stretchDotId}
					style={{
						display: 'block',
					}}
				>
					<input
						...
						type="checkbox"
					/>
					make the '.' character stretch up to the end of the column
					(this is canonical form for CSS grid templates)
				</label>

				<label
					htmlFor={collapseSpacesInputId}
					style={{
						display: 'block',
					}}
				>
					<input
						...
						onChange={e =>
							void setCollapseSpacesFlag(e.target.checked)
						}
						type="checkbox"
					/>
