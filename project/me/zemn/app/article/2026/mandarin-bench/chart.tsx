...
export function MandarinBenchChart() {
	...
	const completeSubmissions = data.rows.filter(row => !row.control).length;
	const controlRows = data.rows.filter(row => row.control).length;
			...
			<div style={{ overflowX: 'auto' }}>
				...
				>
					...
					<text
						...
						fontSize="13"
					>
						{completeSubmissions} complete submissions
						{controlRows > 0 ? ` plus ${controlRows} full human control` : ''};
						stacked bars split each total into Q1-Q{data.questionCount} score
						contributions.
					</text>
					{questionColours.map((fill, index) => (
						...
						['result', 922, 'start'],
							...
							['answers', 1238, 'end'],
							['judge', 1300, 'start'],
						] as const).map(([label, x, textAnchor]) => (
						<text
							key={label}
							x={x}
							y="126"
					...
					{data.rows.map((row, rowIndex) => {
						...
						let segmentOffset = 0;
						const rowScoreMax = row.scoreMax || data.maxScore;
						...
						return (
							<g key={`${row.model}-${row.reasoningEffort}`}>
								<rect
									...
									stroke={row.control ? '#f97316' : 'none'}
									strokeDasharray={row.control ? '4 3' : undefined}
								/>
								<text x="32" y={y} fill="#111827" fontSize="12">
									{row.control ? 'control' : row.rank}
								</text>
								<text
									x="74"
									y={y}
								...
								{row.questions.map((question, questionIndex) => {
									const segmentWidth =
										(question.totalScore / rowScoreMax) * barWidth;
								...
								<text
									...
									fontSize="11"
								>
									{row.control ? 'full control' : row.band}
								</text>
								<text
									x="1238"
									y={y}
