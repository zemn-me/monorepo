...
export const clientSecret = async (s: Storage) => {
	...
	const newKey = crypto.subtle.importKey(
		...
		CryptoKey & { algorithm: Algorithm; usages: ('sign' | 'verify')[] }
	>;
...
function zod<T extends z.ZodTypeAny>(
	schema: T
): Serde<Result<z.output<T>, z.ZodError<z.output<T>>>, string> {
	return [
		v =>
			unwrap_or_else(
