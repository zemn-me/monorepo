...
export default function Header(props: HeaderProps) {
    return <header className={clsx(style.header, props.className)}>
        <TimeEye className={style.logo}/>
        <h2 className={style.name}>
            <div className={style.realName} lang={lang.get(bio.Bio.who.fullName)}>{lang.text(bio.Bio.who.fullName)}</div>
            <div className={style.handle} lang={lang.get(bio.Bio.who.handle)}>{lang.text(bio.Bio.who.handle)}</div>
        </h2>
