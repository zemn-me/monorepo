...
import { ClientSecret, IdToken } from "#root/project/zemn.me/localStorage/localStorage.js";
import { Client } from "#root/project/zemn.me/OAuth/clients.js";
import { and_then as option_and_then, None, Option, Some, unwrap_or as option_unwrap_or, unwrap_or_else as option_unwrap_or_else } from "#root/ts/option/types.js";
...
function SignOnSelector(props: SignOnSelectorProps) {
	...
	return <select defaultValue={""} id={props.id} onChange={onSelectionChange}>
		...
		{
			[...mapping].map(([host, client]) => <option key={host}>
				{client.name}
			</option>)
		}
	</select>
...
function AuthInitiator(props: AuthInitiatorProps) {
	...
	const serverInfo = useQuery({
		...
		queryFn: () => props.client == undefined ?
			undefined : getOpenidConfig(new URL(props.client.issuer))
	});
	...
	const ourURL = option_and_then(
		ourLocation,
		u => new URL(u.href)
	);
	const callbackURL = option_and_then(
		ourURL,
		u => {
			const un = new URL(u.origin);
			un.pathname = u.pathname;
	...
	const params = option_and_then(callbackURL, callback => new URLSearchParams({
