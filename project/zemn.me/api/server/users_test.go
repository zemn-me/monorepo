...
func TestPostAndListAdminUsers(t *testing.T) {
	...
	if created.Email == nil || *created.Email != "testuser@example.com" {
		t.Fatalf("unexpected normalized email: %#v", created.Email)
	}
	...
	if users[0].Id != created.Id {
		t.Fatalf("expected listed user id %q, got %q", created.Id, users[0].Id)
	}
}
...
func TestPutAdminUsersIdUpdatesScopes(t *testing.T) {
	s := newTestServer()
	...
	updateResp, err := s.PutAdminUsersId(context.Background(), PutAdminUsersIdRequestObject{
		Id: created.Id,
		Body: &UpdateUser{
...
func TestDeleteAdminUsersId(t *testing.T) {
	...
	_, err = s.DeleteAdminUsersId(context.Background(), DeleteAdminUsersIdRequestObject{
		Id: created.Id,
	})
	if err != nil {
