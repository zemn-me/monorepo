...
func (s *Server) PostOauth2Token(ctx context.Context, request PostOauth2TokenRequestObject) (ro PostOauth2TokenResponseObject, err error) {
	...
	if mappedIdentity.Picture != "" {
		claims["picture"] = mappedIdentity.Picture
	}
	if mappedIdentity.GivenName != "" {
		claims["given_name"] = mappedIdentity.GivenName
	}
	if mappedIdentity.FamilyName != "" {
		claims["family_name"] = mappedIdentity.FamilyName
	}
	if mappedIdentity.Name != "" {
		claims["name"] = mappedIdentity.Name
	}
