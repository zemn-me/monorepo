...
export function ProgressCircle({
	...
	if (progress < 0 || progress > 1)
		throw new Error(
			'Progress must be between 0 and 1 (was instead ' + progress + ')'
		);
	const width = 1000;
	const height = 1000;
	...
	const handPosX = innerR * Math.cos(-(Math.PI / 2) + 2 * Math.PI * progress);

	const handPosY =
		innerR * Math.sin(-(Math.PI / 2) + 2 * Math.PI * progress) + innerR;
