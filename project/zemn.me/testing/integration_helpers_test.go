...
func waitForOIDCToken(driver selenium.WebDriver, issuer string, timeout time.Duration) (string, error) {
	...
	var lastErr error
	for time.Now().Before(deadline) {
		value, err := driver.ExecuteScript("return localStorage.getItem('1');", nil)
		if err != nil {
			lastErr = err
			time.Sleep(200 * time.Millisecond)
			continue
		}
		cacheString, ok := value.(string)
		if !ok || cacheString == "" {
			time.Sleep(200 * time.Millisecond)
			continue
		}
		...
		if err := json.Unmarshal([]byte(cacheString), &cache); err == nil {
			if entry, ok := cache[issuer]; ok && entry.IDToken != "" {
				return entry.IDToken, nil
			}
		}
		time.Sleep(200 * time.Millisecond)
	}
	...
	if raw, err := driver.ExecuteScript("return localStorage.getItem('1');", nil); err == nil {
		return "", fmt.Errorf("timed out waiting for id_token (cache: %v)", raw)
	}
