...
import { Ok, Result, unwrap as result_unwrap } from '#root/ts/result_types.js';
...
function must<I, O>(s: Serde<Result<I, any>, Result<O, any>>): Serde<I, O> {
	return [
		i => result_unwrap(s[0](Ok(i))),
		i => result_unwrap(s[1](Ok(i)))
	]
...
export const AuthorizationCache = useLocalStorageItem("2", maybe(must(zod(authCacheSchema))))
