		...
		if (typeof window !== 'undefined') {
			try {
				window.localStorage.removeItem(
					`oidc-id-token:${effectiveIssuer}`
				);
			} catch (error) {
				console.warn('Failed to clear persisted OIDC id_token', error);
			}
		}
	}, [effectiveIssuer]);
		...
		(token: string) => {
			...
			if (typeof window !== 'undefined') {
				try {
					...
					console.warn('Failed to persist OIDC id_token', error);
				}
			}
			setLocalToken(token);
		},
		[effectiveIssuer, queryClient, tokenQueryKey]
	);
		...
		if (
			...
			if (typeof window !== 'undefined') {
				try {
					...
					console.warn(
						'Failed to clear persisted OIDC id_token',
						error
					);
				}
			}
