...
export function useOIDC() {
	...
	const authorizationEndpoint =
		oidc_config.status === "success"
			? oidc_config.data.authorization_endpoint.length > 0
				? option.Some(oidc_config.data.authorization_endpoint[0]!)
				: option.None
			: option.None;
	...
	const exchangedTokenRsp = useQuery({
		queryKey: ['oidc-exchange-token', issuer],
		queryFn: option.unwrap_or(option.and_then(
			request_body,
	...
	const exchangedToken =
		exchangedTokenRsp.status === 'success'
			? option.Some(result.unwrap(exchangedTokenRsp.data.error !== undefined
				? result.Err(new Error(exchangedTokenRsp.data.error.error))
				: result.Ok(exchangedTokenRsp.data.data.access_token)))
			: option.None
