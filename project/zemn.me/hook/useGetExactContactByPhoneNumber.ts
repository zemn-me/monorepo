...
import {
	PeopleFieldMask,
	PeoplePhoneNumber,
} from '#root/ts/google/people/display.js';
import {
	concatn_array,
	filter_map,
	iterator,
...
import {
	format_phone_number_e164,
	parse_phone_number,
} from '#root/ts/phone/number.js';
import {
	Err,
	Ok,
	result_and_then,
...
export function useGetExactContactByPhoneNumber(
	...
	const parsed_phone_number = option_and_then_flatten(phoneNumber, phone =>
	...
	const e164_format = option_and_then(parsed_phone_number, p =>
		format_phone_number_e164(p)
	);

	const future = useSearchContact(
		option_unwrap_or(e164_format, ''),
		new Set(['phoneNumbers', ...fields])
	);
	...
	const is_number_we_are_looking_for = option_and_then(
		...
		n => (number: PeoplePhoneNumber) =>
			'canonicalForm' in number && number.canonicalForm === n
	);
