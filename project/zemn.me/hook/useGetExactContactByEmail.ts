...
type PeopleEmailAddress = NonNullable<
...
export function useGetExactContactByEmail(
	...
	const normalized_email = option_and_then_flatten(email, value => {
	...
	const candidates = result_and_then(
		...
		contacts => concatn_array(contacts)
	);
	...
	const matches = result_and_then(candidates, candidates =>
		filter_map(candidates, candidate =>
			candidate.person?.emailAddresses?.some(
				option_unwrap_or(is_email_we_are_looking_for, () => () => false)
			)
				? Some(candidate.person)
				: None
		)
	);

	return result_and_then(matches, m => nth(0)(iterator(m)));
}
