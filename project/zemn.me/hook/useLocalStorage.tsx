...
function onStorageEvent(s: StorageControllerState) {
	return function (e: StorageEventLike): void {
		...
		[...(s.listeners.get(e.key) ?? [])].map(f => f(e.newValue));
	};
}

function setStorageItem(
	s: StorageControllerState,
	key: string,
	value: string | null
) {
	if (value === null) {
...
function stopListeningForStorageEvents(
	state: StorageControllerState
): StorageControllerState {
	if (!state.boundEventListener) throw new Error();
...
function isListeningForStorageEvents(
	state: StorageControllerState
): state is StorageControllerState & {
	boundEventListener: (e: StorageEvent) => void;
} {
	return state.boundEventListener !== undefined;
}

function addStorageKeyListener(
	state: StorageControllerState,
	key: string,
	...
	if (!isListeningForStorageEvents(state))
		state = listenForStorageEvents(state);

	return state;
}

function removeStorageKeyListener(
	state: StorageControllerState,
	key: string,
	callback: (newValue: string | null) => void
) {
	...
	if (state.listeners.size === 0)
		state = stopListeningForStorageEvents(state);
...
function storageController(storage: Storage): Controller {
	...
	return {
		removeItem(key) {
			setStorageItem(state, key, null);
		},
		key(index) {
			return storage.key(index);
		},
		onChange(key, cb) {
			state = addStorageKeyListener(state, key, cb);

			return () => {
				state = removeStorageKeyListener(state, key, cb);
			};
		},
		get length(): number {
		...
		setItem(key, value) {
			setStorageItem(state, key, value);
		},
	};
}

const storageControllerContext = createContext<Option<Controller>>(None);
...
export function LocalStorageController(props: LocalStorageControllerProps) {
	...
	const controller = and_then(storage, s => storageController(s));
	...
	return (
		<storageControllerContext.Provider value={controller}>
			{props.children}
...
export function useLocalStorageItem<T>(
	...
	useEffect(() => {
		and_then(controller, c =>
			LensGet(lens)(Promise.resolve(c)).then(v => setValue(() => Some(v)))
		);
	}, [controller]);
			...
			unwrap_or(
				and_then(
					...
					c =>
						void key.then(k =>
							c.onChange(k, () => {
								void LensGet(lens)(Promise.resolve(c)).then(v =>
									setValue(() => Some(v))
								);
							})
						)
