...
import (
	...
	crane "github.com/zemn-me/monorepo/go/pulumi/provider/crane/provider"
)
...
func TestRandomCreate(t *testing.T) {
	...
	response, err := prov.Create(p.CreateRequest{
		...
		Properties: resource.PropertyMap{
			"path": resource.NewStringProperty("example"),
			"tag":  resource.NewStringProperty("example"),
		},
		Preview: false,
	})
...
func provider() integration.Server {
	return integration.NewServer(crane.Name, semver.MustParse("1.0.0"), crane.Provider())
}
