...
func initializeRealtimeSession(ctx context.Context, oaiConn *websocket.Conn) (err error) {
	...
	// needs to be resampled to mono G.711 ulaw at 8kHz
			...
			V: openai.RealtimeClientEventConversationItemCreate{
				...
				Item: struct {
					...
					Content: &[]struct {
						...
						{
							Type: strPtr("input_text"),
							Text: strPtr("What follows is the conversation with the USER. Do not respond with text. Respond with AUDIO."),
						},
					},
				},
			},
...
func (r *RealtimeServerEventResponse) UnmarshalJSON(b []byte) (err error) {
	...
	case "response.audio.delta":
		...
		r.value = v
	case "error":
		var v *openai.RealtimeServerEventError
		err = json.Unmarshal(b, &v)
		r.value = v
	case "response.done":
	...
	case "response.text.done":
		var v *openai.RealtimeServerEventResponseTextDone
		err = json.Unmarshal(b, &v)
		r.value = v
...
func sendToTwilio(ctx context.Context, wsConn *websocket.Conn, openaiConn *websocket.Conn, streamSid *string) (err error) {
	...
	for {
		...
		default:
			...
			case *openai.RealtimeServerEventError:
				cancel(fmt.Errorf("OpenAI Realtime API Error: %+q", *v.Error.Message))
			case *openai.RealtimeServerEventResponseAudioTranscriptDone:
				log.Println(v.Transcript)
			case *openai.RealtimeServerEventResponseTextDone:
				log.Println("Model responded with text:", v.Text)

			case *openai.RealtimeServerEventResponseOutputItemDone:
