...
func ResolveImportSetToTagList(c *config.Config, importSet []string, ix *resolve.RuleIndex, from label.Label) (l []build.Expr) {
	...
	for _, s := range importSet {
		...
		if len(f) == 0 {
			maybePanic(fmt.Sprintf("Can’t work out an appropriate way to import %+q! in %+q", s, from))
			continue
		}
		l = append(l, &build.StringExpr{
			Value: f[0].Label.String(),
		})
	}
...
func (l Language) Resolve(c *config.Config, ix *resolve.RuleIndex, rc *repo.RemoteCache, r *rule.Rule, imports interface{}, from label.Label) {
	...
	for dep := range imports.(js.DepSet) {
		...
		if len(builtin) > 0 {
			fmt.Println("Resolved as nonlocal", dep)
		...
		if !strings.HasSuffix(dep, ".js") {
			fmt.Println("Known not JS", dep)
			// must be asset if not module and not .js
			assetTags = append(assetTags, dep)
			continue
		}

		fmt.Println("Local JS", dep)

		f := ix.FindRulesByImportWithConfig(c, resolve.ImportSpec{
			Lang: "typescript", Imp: dep,
		}, "typescript")

		if len(f) == 0 {
			maybePanic(fmt.Sprintf("Can’t work out an appropriate way to import %+q! in %+q", dep, from))
			continue
		}

		importTags = append(importTags, &build.StringExpr{Value: f[0].Label.String()})
	}
...
func generateRules(args language.GenerateArgs) (result language.GenerateResult, err error) {
	...
	srcs := stringListToExprList(tsFiles)
