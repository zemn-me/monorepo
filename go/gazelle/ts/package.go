...
func (Language) Configure(c *config.Config, rel string, f *rule.File) {
	if _, ok := c.Exts["typescript"]; ok {
		return
	}

	// this layout is for one canonical package.json, which is what I use.
	fPkgJson, err := os.Open(path.Join(path.Dir(f.Path), "tsconfig.json"))
	if err != nil {
...
func JsImportsForFileModule(c *config.Config, filePath string) (symbols []string) {
	for importFromPat, importToPats := range c.Exts["javascript"].(PackageJsonPartial).Imports {
		for importToPat := range importToPats {
