	...
	for _, src := range srcs.List {
		for _, importSymbol := range TsImportsForFileModule(c.Exts["javascript"].(js.PackageJsonPartial), path.Join(f.Pkg, src.(*build.StringExpr).Value)) {
			for _, lang := range []string{this.Name(), js.Language{}.Name()} {
				fmt.Println(importSymbol, "importable from", lang)
...
func (l Language) Resolve(c *config.Config, ix *resolve.RuleIndex, rc *repo.RemoteCache, r *rule.Rule, imports interface{}, from label.Label) {
	...
	for dep := range imports.(js.DepSet) {
		builtin := js.ResolveNonLocalImportTags(c.Exts["javascript"].(js.PackageJsonPartial), dep)
		if builtin != nil {
...
func generateRule(fileName string, args language.GenerateArgs) (res *GenerateRuleResult, err error) {
	...
	var deps js.DepSet = make(js.DepSet)
...
func (Language) Fix(c *config.Config, f *rule.File) {
	// remove glob callsites for sources
	for _, rule := range f.Rules {
		...
		if _, ok := rule.Attr("srcs").(*build.ListExpr); !ok {
			continue
		}

		rule.SetAttr("srcs", &build.ListExpr{})
