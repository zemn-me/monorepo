...
import (
	...
	gazelleutil "github.com/zemn-me/monorepo/go/gazelle/util"
)
...
func (Language) Name() string { return "mdx" }
...
func (this Language) Imports(c *config.Config, r *rule.Rule, f *rule.File) (specs []resolve.ImportSpec) {
	...
	for _, src := range srcs.List {
		...
		s := src.(*build.StringExpr).Value

		s = strings.TrimSuffix(s, ".mdx") + ".js"

		filePath := filepath.Join(f.Pkg, s)

		// importing the file directly
		for _, symbol := range c.Exts["javascript"].(js.PackageJsonPartial).ResolveJSModule(filePath) {
			for _, lang := range []string{"javascript", "typescript"} {
				fmt.Println(filePath, "importable as", symbol)
				specs = append(specs, resolve.ImportSpec{
					Lang: lang, Imp: symbol,
				})
			}
		}
	}
...
func (Language) CrossResolve(c *config.Config, ix *resolve.RuleIndex, spec resolve.ImportSpec, lang string) (result []resolve.FindResult) {
	...
	fmt.Println("??", spec, lang)

	if spec.Lang == lang && (lang == "javascript" || lang == "typescript") {
		result = ix.FindRulesByImportWithConfig(c, spec, "mdx")
...
func isOfLanguage(name string) bool {
	return strings.HasSuffix(name, ".mdx")
}
...
func generateRules(args language.GenerateArgs) (result language.GenerateResult, err error) {
	var files []string = make([]string, 0, len(args.RegularFiles))
	...
	srcs := gazelleutil.StringListToExprList(files)
