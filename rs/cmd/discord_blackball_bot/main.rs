...
impl Handler {
    ...
    async fn get_or_create_applicant_role(&self, ctx: Context, guild: GuildId) -> Result<Role> {
        let x = self.get_role_by_name(ctx, guild, "applicant".into()).await?
            .unwrap_or_else(|| guild.create_role(&ctx.http,
                EditRole::new().name("applicant")
            ));
    ...
    async fn handle_message(&self, ctx: Context, msg: Message) -> Result<()> {
        ...
        if !msg.author.has_role(&ctx.http, msg.guild_id, )
