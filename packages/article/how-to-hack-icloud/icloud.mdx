...
| §5 | [A Big, Slightly Off-White Box]  |
| §6 | [Analysing the 'Beefy Boy'] |
| §7| [PostMessage Fans, You'd Be Correct.] |
| §8| [It's Not Always That You Need to Read The Spec, But That Always is Today.]  |
| §9 | [Stealing the Bank From an ATM] |
| §10 | [Conclusion] |

**By Exploit / Impact**

1. URL Manipulation ([§3])
    * Inject extra unsanitized characters into `redirect_uri` while maintaining HTTP origin
2. Content Security Policy injection ([§4])
    * Remove / add / replace Content Security Policy controls preventing cross-site scripting attacks
    * Remove / add / replace `frame-ancestors` directives, allowing us to embed the Apple ID login wherever we want.  
