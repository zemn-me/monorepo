...
describe('fixedTimeStringEquals', () => {
	it('returns true for identical ASCII strings', async () => {
		await expect(fixedTimeStringEquals('hello', 'hello')).resolves.toBe(
			true
		);
	});

	it('returns false for different ASCII strings', async () => {
		await expect(fixedTimeStringEquals('hello', 'world')).resolves.toBe(
			false
		);
	});
	...
	it('returns false for same prefix but different tail', async () => {
		await expect(
			fixedTimeStringEquals('token-abc123', 'token-abc124')
		).resolves.toBe(false);
	});

	it('handles different lengths', async () => {
		await expect(fixedTimeStringEquals('short', 'shorter')).resolves.toBe(
			false
		);
		await expect(fixedTimeStringEquals('longer', 'long')).resolves.toBe(
			false
		);
	});
	...
	it('works with multibyte (Unicode) strings', async () => {
		...
		await expect(fixedTimeStringEquals('café', 'cafe')).resolves.toBe(
			false
		);
	});
	...
	it('distinguishes composed vs decomposed Unicode forms', async () => {
		...
		await expect(fixedTimeStringEquals(composed, decomposed)).resolves.toBe(
			false
		);
	});
