...
export const Command =
	...
	(parameters: CommandValidation[T]) =>
	(line?: string) =>
		// the title injection here is because GitHub, in its ultimate wisdom, does not actually display
		// the title inline in the error log. I know! How strange.
		//
		// Instead, it just displays the first line of the content. So we put that in there.
		commandIdent(command)(parameters) +
		escapeData(
			(parameters?.title !== undefined ? parameters.title + '\n' : '') +
				line
		);
