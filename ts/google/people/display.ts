...
export type PeopleName = NonNullable<
	components['schemas']['Person']['names']
>[number];

export type PeopleFieldMask = keyof PeoplePerson;

export type PeoplePhoneNumber = NonNullable<
	PeoplePerson['phoneNumbers']
>[number];

function formatPeopleName(n: PeopleName): string {
 ...
 */
export function nicknames(person: PeoplePerson) {
	return filter(
		map(person.nicknames ?? [], nickname =>
			nickname.value !== undefined ? Some(nickname) : None
		)
	);
}
...
export function displayPersonName(
	...
	const defaultNickname = nth(0)(
		iterator(
			filter_map(person?.nicknames ?? [], nick =>
				nick.value !== undefined && nick.metadata?.primary === true
					? Some(nick)
					: None
			)
		)
	...
	const otherNicknames = nth(0)(
		iterator(
			filter_map(person?.nicknames ?? [], nick =>
				nick.value !== undefined ? Some(nick) : None
			)
		)
	);
	...
	const defaultNames = nth(0)(
		iterator(
			filter_map(person?.names ?? [], name =>
				name.metadata?.primary === true ? Some(name) : None
			)
		)
	);
	...
	return and_then(nth(0)(iterator(person?.names ?? [])), name =>
		formatPeopleName(name)
	);
}
