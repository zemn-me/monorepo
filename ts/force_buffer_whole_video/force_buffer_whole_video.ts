...
function didVideoBufferGrow(video: HTMLVideoElement, sampleTimeMs: number): Promise<boolean> {
    const initialSize = totalBufferedSize(video);
    return new Promise(ok => {
        setTimeout(() => ok(totalBufferedSize(video) > initialSize), sampleTimeMs)
    });
}

async function awaitBufferingStarted(video: HTMLVideoElement, sampleTimeMs: number): Promise<void> {
    while(!(await didVideoBufferGrow(video, sampleTimeMs))) {}
}

async function awaitBufferingStopped(video: HTMLVideoElement, sampleTimeMs: number): Promise<void> {
    while(await didVideoBufferGrow(video, sampleTimeMs)) {}
}
...
async function forceBufferWholeVideo(video: HTMLVideoElement) {
    while (!videoTotallyBuffered(video)) {
        console.info(`loaded ${totalBufferedSize(video)*100/video.duration}% in ${video.buffered.length} chunks.`);
