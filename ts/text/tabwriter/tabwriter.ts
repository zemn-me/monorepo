...
/** Public entry‑point. */
export const elasticTabstops = (input: string, opts: Options = {}): string =>
	renderLines(format(parse(input), opts), opts);
...
const format = (
	...
	return [
		...
		...format(lines.slice(firstRow, row), o, col + 1, [
			...prefixWidths,
			width,
		]),
...
const blockToLines = (
	...
	return block.map(line => {
		const cells = line.map((cell, idx) => {
			const colPad =
				idx < prefixWidths.length
					? pad(cell.width, prefixWidths[idx]!, o)
					: '';
			return o.alignRight ? colPad + cell.text : cell.text + colPad;
		});
