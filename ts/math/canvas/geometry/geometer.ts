...
export function normal(
	...
	const normalLength = cartesian.scale(unitNormal, length);
...
export interface AnnotateResult {
	columns: [
		leftColumn: cartesian.Line2D<2>,
		rightColumn: cartesian.Line2D<2>,
	];
	textCentre: cartesian.Point2D;
	arrows: [leftArrow: cartesian.Line2D<2>, rightArrow: cartesian.Line2D<2>];
}
...
export function annotate(
	...
	const leftCol = normal(l, pt1, columnHeight);

	const rightCol = normal(l, pt2, columnHeight);
	...
	const arrowArea = nonColumnArea - props.textPadding * 2 - props.textWidth;

	const arrowLength = arrowArea / 2;

	const leftArrowStartPt: cartesian.Point2D = cartesian.add<1, 2>(
		leftCol[1],
		cartesian.scale(parallelUnit, props.arrowGap)
	);

	const leftArrowEndPt: cartesian.Point2D = cartesian.add<1, 2>(
		leftArrowStartPt,
		cartesian.scale(parallelUnit, arrowLength)
	);
	...
	const textStartPt: cartesian.Point2D = cartesian.add<1, 2>(
		leftArrowEndPt,
		cartesian.scale(parallelUnit, props.textPadding)
	);

	const textEndPt: cartesian.Point2D = cartesian.add<1, 2>(
		textStartPt,
		cartesian.scale(parallelUnit, props.textWidth)
	);
