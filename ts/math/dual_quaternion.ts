...
export const transformPoint2 = (
	...
	const rotated = pipe_result(rotation, r => Quaternion.rotateVector(r, p));
	const inverseRotation = pipe_result(rotation, Quaternion.inverse);

	const translationQuat = and_then(inverseRotation, invRot =>
		Quaternion.multiply(dual(dq), invRot)
	);

	return zipped(rotated, translationQuat, (rotated, translationQuat) => {
...
export const transformPoint = (
	dq: DualQuaternion,
	p: Point3D
): Result<Point3D, Error> => transformPoint2(dq, p);
...
export const fromRotationTranslation = (
	...
	pipe(
		...
		map_result(unitRotation => {
			...
			const dualPart = Quaternion.multiply(
