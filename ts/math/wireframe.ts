...
import { and_then_flatten, Ok, result_collect, type Result } from '#root/ts/result/result.js';
		...
		transform => result_collect(
			segments.map(([start, end]) =>
				and_then_flatten(
					...
					nextStart => and_then_flatten(
						DualQuaternion.transformPoint(transform, end),
						nextEnd => Ok([nextStart, nextEnd] as Segment3D)
					)
				)
			)
		)
