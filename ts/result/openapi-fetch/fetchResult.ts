...
type FailureResponse<T extends Record<string | number, any>> = ErrorResponse<
	ResponseObjectMap<T>
>;
...
): Result<ParseAsResponse<SuccessResponse<ResponseObjectMap<A>, C>, B>, E> {
	if (r.error !== undefined) return Err(err(r.error));
