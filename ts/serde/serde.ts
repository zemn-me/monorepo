...
export function serdeIdentity<T>(): Serde<T, T> {
	return [/** serialize */ (v: T) => v, /** deserialize */ (v: T) => v];
}
...
export function serdeKey<
	const A extends Algorithm,
	U extends CryptoKey['usages'],
>(
	a: Algorithm,
	usages: U
	...
	return [
		async k =>
			JSON.stringify(await crypto.subtle.exportKey('jwk', await k)),
		async s =>
			crypto.subtle.importKey(
				...
				true,
				usages
			) as Promise<CryptoKey & { algorithm: A; usages: U }>,
	];
}

export function asyncSerdeNullable<I, O>(
	s: Serde<Promise<I>, Promise<O>>
): Serde<Promise<I | null>, Promise<O | null>> {
	return [
		/** serialize */ async v => {
			const vv = await v;
...
export function serdeOptional<I, O>(
	s: Serde<I | null, O | null>
): Serde<Option<I>, Option<O>> {
	return [
		/** serialize */ v => {
...
export function serdePromise<I, O>(
	s: Serde<I, O>
): Serde<Promise<I>, Promise<O>> {
	return [
...
export function serdeLens<Haystack, Needle, Deserialized>(
	...
	return f =>
		f(
			...
			(needle, haystack) =>
				LensSet(lens)(Serialize(serde)(needle), haystack)
		);
}
