...
program
	...
	.option(
		'--throttle <n>',
		'Maximum number of pages to load at once. set to `1` for sequential operation',
		10
	)
	.option(
		'--block',
...
) => EventuallyIterable<O> = async function* (f, iter) {
	let n = 0;
...
type RepeatTuple<N extends number, T> = N extends keyof RepeatTupleMap<T>
	? RepeatTupleMap<T>[N]
	: readonly T[];

const chunk: <N extends number>(
	size: Eventually<N>
) => <T>(v: EventuallyIterable<T>) => EventuallyIterable<RepeatTuple<N, T>> =
	<N extends number>(size: Eventually<N>) =>
	<T>(iter: EventuallyIterable<T>) =>
		(async function* () {
			let bucket: T[] = [];
...
const EventuallyIterable: <T>(
	I: EventuallyIterable<Eventually<T>>
) => EventuallyIterable<T> = async function* <T>(
	I: EventuallyIterable<Eventually<T>>
): EventuallyIterable<T> {
	for await (const value of I) yield await value;
};
/** perform a promise iterator lazily in chunks */
const chunkedPromise: <N extends number>(
	N: Eventually<N>
...
const flat: <T>(
	I: EventuallyIterable<EventuallyIterable<T>>
) => EventuallyIterable<T> = async function* (I) {
	for await (const chunk of I) for await (const member of chunk) yield member;
};
/** lazily completes the given async iterable in chunks of given size */
const throttle: <N extends number>(
	N: N
) => <T>(I: EventuallyIterable<T>) => EventuallyIterable<T> = N => I =>
	flat(EventuallyIterable(chunkedPromise(N)(I)));
const main = async () => {
	...
	const captures = map(async (url, i) => {
		...
		const title = ((await page.title()).trim() || page.url()).replace(
			/[^A-z_-]/g,
			'_'
		);
	...
	for await (const _ of throttle(throttleN)(captures));
