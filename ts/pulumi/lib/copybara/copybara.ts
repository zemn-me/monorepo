...
export class Copybara extends ComponentResource {
	...
	constructor(
		...
		super('ts:pulumi:lib:copybara:copybara', name, args, opts);

		const interpreterArgs = all([
			output(args.args ?? []),
			args.configPath,
		]).apply(([args, configPath]) => [configPath, ...args]);
		...
		const run = new SafeCommand(
			`${name}_run`,
			{
				environment: output(args.githubToken).apply(GITHUB_TOKEN =>
					GITHUB_TOKEN
						? ({ GITHUB_TOKEN } as {
