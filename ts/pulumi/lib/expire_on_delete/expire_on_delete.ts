...
import { ExpireOnDeleteLambdaImage } from "#root/ts/pulumi/lib/expire_on_delete/cmd/expire_on_delete/ExpireOnDeleteLambdaImage";
...
export class S3ExpireOnDeletePolicy extends ComponentResource {
    ...
    constructor(name: string, args: S3ExpireOnDeletePolicyArgs, opts?: ComponentResourceOptions) {
        ...
        // IAM role for Lambda
        const lambdaRole = new aws.iam.Role(`${name}_Role`, {
        ...
        const lambdaBucketPolicy = new aws.iam.Policy(`${name}-lambdaBucketPolicy`, {
            ...
            policy: output(args.bucketId).apply(bucketArn =>
                JSON.stringify({
                    Version: "2012-10-17",
                    Statement: [
                        {
