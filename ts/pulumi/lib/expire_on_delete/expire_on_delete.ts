...
const sanitisedStatementId = (name: string) =>
...
export class S3ExpireOnDeletePolicy extends ComponentResource {
	...
	constructor(
		name: string,
		args: S3ExpireOnDeletePolicyArgs,
		...
		const bucket = output(args.bucketId).apply(i =>
			aws.s3.getBucket({
				bucket: i,
			})
		);
		...
		if (!image) {
			image = new ExpireOnDeleteLambdaImage(
				`expireOnDeleteLambdaImage`,
				{
		...
		const lambdaBucketPolicy = new aws.iam.Policy(
			`${name}-lambdaBucketPolicy`,
			{
		...
		new aws.iam.RolePolicyAttachment(
			`${name}-lambdaBucketPolicyAttachment`,
			{
				role: lambdaRole.name,
		...
		const allowPerm = new aws.lambda.Permission(
			clampString(
				sanitisedStatementId(`${name}-lambdaS3InvokePermission`),
				100 - 10
			),
			{
