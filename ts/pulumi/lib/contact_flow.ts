...
interface EqualsCondition extends Condition {
	Operator: 'Equals';
	Operands: [string];
}
...
interface GetParticipantInputAction extends ActionBase {
	...
	Transitions?: ActionBase['Transitions'] & {
		Errors?: {
			NextAction: string;
			ErrorType:
				| 'NoMatchingCondition'
...
export interface InvokeLambdaFunctionAction extends ActionBase {
	...
	Parameters: {
		/**
		 * The ARN of the AWS Lambda function to be invoked. May be defined statically or dynamically.
		 */
		LambdaFunctionARN: string;
		/**
		 * The number of seconds to wait for a response from the AWS Lambda function. Must be greater than 0, no larger than 8, and an integer. Must be set statically.
		 */
		InvocationTimeLimitSeconds: number;
		/**
		 *  A map of additional data to send to the AWS Lambda function when invoking it. Keys and values may be set statically or dynamically.
		 */
		LambdaInvocationAttributes: Record<string, string>;
		ResponseValidation: {
			/**
			 * If response validation is set to STRING_MAP, then the lambda function should return a flat object of key/value pairs of the string type. Otherwise, if response validation is set to JSON, the lambda function can return any valid JSON including nested JSON. Must be set statically.
			 */
