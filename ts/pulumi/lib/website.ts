...
import { zone } from "../dog/pleaseintroducemetoyour";

function relative(from: string, to: string): string {
    const f = path.normalize(from), t = path.normalize(to);
...
export class Website extends pulumi.ComponentResource {
    constructor(name: string, args: Args, opts?: pulumi.ComponentResourceOptions) {
        ...
        const errorDocument = relative(args.directory, args.notFound);
        ...
        const uploadDir = (dir: string): Map<string, aws.s3.BucketObject> => {
            const out: Map<string, aws.s3.BucketObject> = new Map();
            for (const item of fs.readdirSync(dir)) {
                ...
                if (fileInfo.isDirectory()) {
                    return uploadDir(fPath);
                }

                out.set(fPath, new aws.s3.BucketObject(`${name}_bucket_file_${fPath}`, {
                    key: relative(args.directory, fPath),
                    bucket,
                    contentType: guard.must(guard.isNotNull)(mime.getType(fPath)),
                    source: fPath,
                    acl: 'public-read',
                }));
            }
        ...
        const errorDocumentObject = guard.must(guard.isDefined)(objects.get(args.notFound));
        const indexDocumentObject = guard.must(guard.isDefined)(objects.get(args.index));
        // create the cloudfront

        const distribution = new aws.cloudfront.Distribution(`${name}_cloudfront_distribution`, {
                origins: [
                    {
