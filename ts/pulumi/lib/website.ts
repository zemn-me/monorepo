...
async function relative(from: Promise<string>, to: Promise<string>): Promise<string> {
...
export class Website extends pulumi.ComponentResource {
    constructor(name: string, args: Args, opts?: pulumi.ComponentResourceOptions) {
        ...
        const bucket = new aws.s3.Bucket(`${name}_bucket`, {
            ...
            website: {
                indexDocument: 
                    relative(args.directory.path, args.index.path),

                errorDocument:
                    relative(args.directory.path, args.notFound.path)
            }
        })
