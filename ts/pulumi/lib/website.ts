...
export class Website extends pulumi.ComponentResource {
	constructor(
			...
			for (const item of fs.readdirSync(dir)) {
				...
				out.set(
					...
					new aws.s3.BucketObject(`${name}_bucket_file_${fPath}`, {
						...
						bucket,
						contentType: guard.must(guard.isNotNull)(
							mime.getType(fPath)
						),
						source: fPath,
						acl: 'public-read',
		...
		const errorDocumentObject = guard.must(guard.isDefined)(
			objects.get(args.notFound)
		);
		const indexDocumentObject = guard.must(guard.isDefined)(
         ...
         */
        const targetDomain = args.zone.name.apply(
            zoneName => [args.subDomain, args.zone.name].filter(guard.isDefined).join(".")
        )
