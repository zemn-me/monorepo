...
export async function main() {
	...
	switch (true) {
		case upError === undefined && destroyError !== undefined:
			e = new Error(
				'A deployment to staging completed successfully, but ' +
		...
		case upError !== undefined && destroyError !== undefined:
			e = new Error(
				'Both attempting to create the infrastructure, and attempting to destroy it failed.'
			);
		...
		case upError !== undefined && destroyError == undefined:
			e = new Error(
				'Attempting to create the infrastructure succeeded. However attempting to destroy it failed.'
			);
			e.cause = upError;
			break;
		case upError === undefined && destroyError === undefined:
