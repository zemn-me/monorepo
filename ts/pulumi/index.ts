...
class StagableZone extends Pulumi.ComponentResource {
	...
	constructor(
		...
		this.zone = new aws.route53.Zone(
			`${name}_zone`,
			{
				name: args.staging ? `staging.${args.name}` : args.name,
			},
			{ parent: this }
		);
...
export class Component extends Pulumi.ComponentResource {
	...
	constructor(
		...
		const pleaseIntroduceMeToYourDogZone = new StagableZone(
			`${name}_pleaseintroducemetoyour.dog_zone`,
			{
		...
		const zemnMeZone = new StagableZone(
			`${name}_zemn.me_zone`,
			{
				name: 'zemn.me',
				staging: args.staging,
		...
		const shadwellImZone = new StagableZone(
			`${name}_shadwell.im_zone`,
			{
				name: 'shadwell.im',
				staging: args.staging,
			},
