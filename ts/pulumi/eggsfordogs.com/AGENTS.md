...
Use `bazel test //ts/pulumi/eggsfordogs.com:park_benchmark --test_output=all --nocache_test_results` for browser frame intervals/DOM mutations, and `bazel run //ts/pulumi/eggsfordogs.com/app:park_bench` for deterministic CPU stages. `service_test` needs benchmark arguments on the wrapper, not the underlying `go_test`.
