...
export function EggDogYardClient() {
	...
	useEffect(() => {
		...
		const loop = (now: number) => {
			...
			const lookedPose =
				!locked && (lookDelta.x !== 0 || lookDelta.y !== 0)
			...
			const nextCritters = stepCritters(
				crittersRef.current,
				dt,
				now / 1000
			);
	...
	const projection = perspective(viewport.width, viewport.height, {
		focalScale: 0.75,
	});
	const segments = unwrap(renderSegments(world.scene, pose, projection));
	const sprites = critters
		.map(critter => {
			const raised = point<3>(
				x(critter.position),
				1.1,
				z(critter.position)
			);
			const projected = unwrap(
				projectWorldPoint(raised, pose, projection)
			);
		...
		>
			...
			>
				...
				<line
					...
					strokeLinecap="round"
					strokeWidth={1.5}
					x1={viewport.width / 2 - 12}
					x2={viewport.width / 2 + 12}
				...
				<line
					...
					x1={viewport.width / 2}
					x2={viewport.width / 2}
					y1={viewport.height / 2 - 12}
					y2={viewport.height / 2 + 12}
				/>
			</svg>
			<div
				style={{
			...
			>
				<button
					...
					style={{
						background: '#fff7d6',
						border: '1px solid rgb(59 51 38 / 24%)',
						borderRadius: '6px',
						color: '#3b3326',
						cursor: 'pointer',
				...
				<div
					style={{
						background: 'rgb(255 247 214 / 82%)',
						border: '1px solid rgb(59 51 38 / 18%)',
						borderRadius: '6px',
						color: '#3b3326',
				...
				>
					<div
						...
						style={{
							...
							background:
								'radial-gradient(circle at center, rgb(255 244 197 / 36%) 0, rgb(255 244 197 / 22%) 34%, rgb(110 140 72 / 44%) 70%, rgb(76 103 45 / 58%) 100%)',
							border: '1px solid rgb(59 51 38 / 18%)',
							borderRadius: '999px',
							boxShadow:
								'inset 0 0 0 1px rgb(255 255 255 / 18%), 0 0.8rem 1.8rem rgb(0 0 0 / 12%)',
							height: '5.5rem',
							pointerEvents: 'auto',
							position: 'relative',
							touchAction: 'none',
					...
					>
						<div
							style={{
								background: 'rgb(255 250 229 / 28%)',
								border: '1px solid rgb(59 51 38 / 14%)',
								borderRadius: '999px',
								height: '1.2rem',
								left: '50%',
								marginLeft: '-0.6rem',
						...
						<div
							style={{
								background:
									'linear-gradient(180deg, rgb(255 251 239 / 94%) 0%, rgb(242 213 127 / 82%) 100%)',
								border: '1px solid rgb(59 51 38 / 24%)',
								borderRadius: '999px',
								boxShadow:
									'0 0.35rem 0.8rem rgb(0 0 0 / 20%), inset 0 0 0 1px rgb(255 255 255 / 35%)',
								height: '2.35rem',
								left: '50%',
								marginLeft: '-1.175rem',
								marginTop: '-1.175rem',
					...
					<div
						...
						style={{
							backdropFilter: 'blur(10px)',
							background:
								'radial-gradient(circle at center, rgb(190 236 255 / 34%) 0, rgb(190 236 255 / 20%) 34%, rgb(130 166 124 / 40%) 70%, rgb(79 112 84 / 54%) 100%)',
							border: '1px solid rgb(59 51 38 / 18%)',
							borderRadius: '999px',
							boxShadow:
								'inset 0 0 0 1px rgb(255 255 255 / 18%), 0 0.8rem 1.8rem rgb(0 0 0 / 12%)',
					...
					>
						...
						<div
							style={{
								background:
									'linear-gradient(180deg, rgb(240 250 255 / 94%) 0%, rgb(145 205 229 / 82%) 100%)',
								border: '1px solid rgb(59 51 38 / 24%)',
								borderRadius: '999px',
								boxShadow:
									'0 0.35rem 0.8rem rgb(0 0 0 / 20%), inset 0 0 0 1px rgb(255 255 255 / 35%)',
