...
describe('pulumi', () => {
	...
	test('smoke', async () => {
		...
		const minecraftContainerDefinitions =
			minecraftTaskDefinition?.inputs['containerDefinitions'];
		expect(typeof minecraftContainerDefinitions).toBe('string');
		const minecraftContainer = (
			JSON.parse(
				minecraftContainerDefinitions as string
			) as EcsContainerDefinition[]
		).find(container => container.name === 'minecraft');
		expect(minecraftContainer?.image).toBe('itzg/minecraft-server:latest');
		expect(
			minecraftContainer?.environment?.some(
				variable => variable.name === 'VERSION'
			)
		).toBe(false);
		...
		const minecraftWakeNames = (
			(
				minecraftWakeFunction?.inputs['environment'] as
					| { variables?: Record<string, string> }
					| undefined
			)?.variables?.['WAKE_NAMES'] ?? ''
		).split(',');
		expect(minecraftWakeNames).toEqual([
			'minecraft.zemn.me',
			'minecraft.zemn.me.',
			'_minecraft._tcp.zemn.me',
			'_minecraft._tcp.zemn.me.',
			'_minecraft._tcp.minecraft.zemn.me',
			'_minecraft._tcp.minecraft.zemn.me.',
		]);
		expect(minecraftWakeNames).not.toContain('zemn.me');
		expect(minecraftWakeNames).not.toContain('zemn.me.');
		...
		const minecraftRootLogGroup = mockResources.find(
			resource =>
				resource.type === 'aws:cloudwatch/logGroup:LogGroup' &&
				resource.name.endsWith('_minecraft_root_dns_query_logs')
		);
		expect(minecraftRootLogGroup?.inputs['name']).toBe(
			'/aws/route53/zemn.me/minecraft-wake'
		);

		const minecraftRootQueryLog = mockResources.find(
			resource =>
				resource.type === 'aws:route53/queryLog:QueryLog' &&
				resource.name.endsWith('_minecraft_root_query_log')
		);
		expect(minecraftRootQueryLog?.inputs['zoneId']).toBe('ZG99IJ8QHXTSI');
		...
		expect(
			minecraftSrvRecords.map(resource => resource.inputs['name']).sort()
		).toEqual([
			'_minecraft._tcp.minecraft.zemn.me',
			'_minecraft._tcp.zemn.me',
		]);
		for (const record of minecraftSrvRecords) {
			expect(record.inputs['records']).toEqual([
				'0 0 25565 server.minecraft.zemn.me.',
			]);
		}
