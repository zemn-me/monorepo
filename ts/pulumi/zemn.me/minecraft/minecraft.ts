...
export class MinecraftOnDemand extends Pulumi.ComponentResource {
	constructor(
		...
		const wakeNames = [
			publicDomain,
			`${publicDomain}.`,
			...srvPlayerDomains.flatMap(domain => [
		...
		if (manageDnsWake) {
			...
			const rootQueryLogGroup = new aws.cloudwatch.LogGroup(
				resourceName('root_dns_query_logs'),
				{
					name: `/aws/route53/${args.domain}/minecraft-wake`,
					retentionInDays: 3,
					tags,
				},
				{ parent: this }
			...
			new aws.route53.QueryLog(
				resourceName('root_query_log'),
				{
					cloudwatchLogGroupArn: rootQueryLogGroup.arn,
					zoneId: args.zoneId,
				},
				{ parent: this, dependsOn: [route53LogPolicy] }
			);
			...
			new aws.cloudwatch.LogSubscriptionFilter(
				resourceName('root_dns_query_filter'),
				{
					destinationArn: wakeFunction.arn,
					filterPattern: 'minecraft',
					logGroup: rootQueryLogGroup.name,
				},
				{ parent: this, dependsOn: [logsPermission] }
			);
		}
