...
function requireFirstSubnet(ids: string[]): string {
	const id = ids[0];
	if (!id) throw new Error('default VPC has no subnets');
	return id;
}
...
export class MinecraftOnDemand extends Pulumi.ComponentResource {
	constructor(
		...
		const firstSubnetId = Pulumi.output(defaultSubnetIds).apply(requireFirstSubnet);
		...
		const taskDefinition = new aws.ecs.TaskDefinition(
			`${name}-task`,
			{
				...
				containerDefinitions: Pulumi.all([
					...
					([logGroupName, operators, whitelist]) => {
						const allowList = [...new Set([...operators, ...whitelist])];
