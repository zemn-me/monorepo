...
export class MinecraftOnDemand extends Pulumi.ComponentResource {
	...
	constructor(
		...
		new aws.route53.Record(
			...
			{ parent: this, ignoreChanges: ['records'] }
		);
							...
							JSON.stringify({
								...
								Statement: [
									...
									},
									{
										...
										Resource: [
											`arn:aws:route53:::hostedzone/${publicZoneId.replace(/^\/hostedzone\//, '')}`,
											`arn:aws:route53:::hostedzone/${privateZoneId.replace(/^\/hostedzone\//, '')}`,
										],
									},
								],
							})
			...
			resourceName('wake'),
			{
				...
				environment: {
					variables: {
						...
						PRIVATE_DNS_HOSTED_ZONE_ID: privateDnsZone.zoneId,
