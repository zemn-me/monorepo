...
export class GcpWorkstation extends pulumi.ComponentResource {
	constructor(
		...
		const apiService = new gcp.projects.Service(
			'forgeapiservice',
			{
		...
		const cluster = new gcp.workstations.WorkstationCluster(
			'forgecluster',
			{
		...
		const config = new gcp.workstations.WorkstationConfig(
			'forgeconfig',
			{
		...
		const ws = new gcp.workstations.Workstation(
			'forgews',
			{
		...
		new gcp.workstations.WorkstationIamMember(
			'forgews-user-binding',
			{
				location: args.location,
				workstationClusterId: cluster.workstationClusterId,
				workstationConfigId: config.workstationConfigId,
