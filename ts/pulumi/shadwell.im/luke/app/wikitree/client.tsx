...
function getCacheStore() {
	...
	try {
		...
		const parsed = JSON.parse(raw) as Record<string, CachedEntityRecord>;
...
async function buildGraph(
	...
	const emitGraphUpdate = (force = false) => {
		if (!onGraphUpdate) {
			return;
		}
...
function drawClusterFamilyLabels(
	...
	for (let startIndex = 0; startIndex < visibleNodes.length; startIndex++) {
		...
		const matchingNodes = clusterIndices
			...
			.filter(node => node.surname === dominantSurname);
...
export default function WikiTreePage() {
	...
	function handlePointerDown(event: PointerEvent<HTMLDivElement>) {
		...
		dragStateRef.current = {
			moved: false,
			node: pickNode(event.clientX - rect.left, event.clientY - rect.top),
		...
		if (dragStateRef.current.node) {
			dragStateRef.current.mode = 'node';
			dragStateRef.current.node.fx = dragStateRef.current.node.x;
			dragStateRef.current.node.fy = dragStateRef.current.node.y;
					...
					<section className="panel graphPanel">
						<div className="graphHeader">
							...
							<div className="graphActions">
								...
								{selectedNode ? (
									<a href={selectedNode.url} rel="noreferrer" target="_blank">
						...
						<div className="filterPanel">
							...
							<div className="filterBar" role="toolbar" aria-label="Highlight filters">
						...
						{!graph ? (
							...
							>
								...
								<div className="labelLayer" ref={labelContainerRef} aria-hidden="true" />
