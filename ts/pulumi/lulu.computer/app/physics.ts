...
) => (self: P, _: Set<P>, _: number): Force => {
	const min: Vector<2> = [minX, minY];
	const max: Vector<2> = [maxX, maxY];

	return contains(Position)(
		minX, minY, maxX, maxY
	)(self) ?

		[0, 0] : // not outside viewport, therefore no force
...
export function simulate<P>(
	particles: Set<P>,
	simStep: SimulationStep<P>,
	Velocity: Property<P, Vector<2>>,
	Mass: Property<P, number>,
	dt: number,
): Set<P> {
