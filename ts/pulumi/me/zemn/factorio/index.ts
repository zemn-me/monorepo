...
const factorioPorts = [
    { protocol: "udp", fromPort: 34197, toPort: 34197, cidrBlocks: [ "0.0.0.0/0" ] },
    { protocol: "tcp", fromPort: 27015, toPort: 27015, cidrBlocks: [ "0.0.0.0/0" ] }
]

export const securityGroup = new aws.ec2.SecurityGroup("factorio-server-security-group", {
    ingress: [
        ...factorioPorts
    ]
});
...
export const startupScript = new pulumi.asset.FileAsset('ts/pulumi/me/zemn/factorio/startupScript.sh')
export const defaultSaveFile = new pulumi.asset.FileAsset('ts/pulumi/me/zemn/factorio/map.zip');
...
export const ec2 = new aws.ec2.Instance("factorio-server", {
    ...
    userData: startupScript.path.then(async path => (await fs.readFile(path)).toString('utf-8'))
});

export const factorioDataMount = new aws.ec2.VolumeAttachment("factorio-server-data-mount", {
    deviceName: '/dev/sdh',
