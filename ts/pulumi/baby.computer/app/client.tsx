...
function encounterCopy(distance: number): string {
	if (distance < 2.2) {
		return 'You are close enough to trade a solemn nod.';
	}

	if (distance < 4.5) {
		return 'Close enough to introduce yourself without shouting over the wind.';
	}

	return 'Still a bit of a trudge across the ice.';
}
...
export function PenguinSim() {
	...
	const encounter = nearestPenguin(world.penguins, pose.position);
	...
	const encounterAnchor = encounter == null
		? null
		: projectWorldPoint(
			point<3>(
				x(encounter.position),
				y(encounter.position) + 2.35,
				z(encounter.position)
			),
			pose,
			viewportSize.width,
			viewportSize.height
					...
					{encounter != null && encounterAnchor != null ? (
						...
						>
							<div className={style.encounter}>
								...
								<p>{encounter.blurb}</p>
								<p>{encounterCopy(encounter.distance)}</p>
							</div>
						</foreignObject>
					) : null}
					<line
