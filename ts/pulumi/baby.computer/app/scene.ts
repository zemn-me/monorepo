...
function applyTransform(
	...
	return rigidTransform(segments, rotation, translation).map(([start, end]) =>
		worldSegment(start, end, stroke, width, opacity)
	);
}
...
function verticalRibs(
	...
	return top.map(
		vertex => [vertex, point<3>(x(vertex), bottomY, z(vertex))] as const
	);
}
...
function scaledPenguinShape(scale: number): ReturnType<typeof penguinShape> {
	...
	return {
		body: shape.body.map(
			([start, end]) =>
				[scalePoint3D(start, scale), scalePoint3D(end, scale)] as const
		),
...
function penguinBody(penguin: Penguin, facing: Point3D): PenguinBody {
	...
	return {
		centre: translate(
			position,
			point<3>(0, penguin.heightM * 0.55, 0)
		) as Point3D,
		outline: shape.fill.map(
			vertex =>
				translate(
					Quaternion.rotateVector(rotation, vertex),
					position
				) as Point3D
		),
	};
