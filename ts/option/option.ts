...
class impl<T> extends NewType<T> {
	 ...
	 */
	/*#__NO_SIDE_EFFECTS__*/
	is_none<T>(this: Option<T>): this is None {
		return types.is_none(this.value);
	}
	 ...
	 */
	/*#__NO_SIDE_EFFECTS__*/
	is_some<T>(this: Option<T>): this is Some<T> {
		return types.is_some(this.value);
	}
	 ...
	 */
	/*#__NO_SIDE_EFFECTS__*/
	unwrap_or<T1, T2>(this: Option<T1>, fallback: T2): T1 | T2 {
	...
	ok_or<T, E>(this: Option<T>, err: E): Result<T, E> {
		return new resultImpl(types.ok_or<T, E>(this.value, err));
	}
	...
	from<T>(this: Some<T | undefined>): Option<T> {
		return new impl(types.from<T>(this.unwrap()));
	}
	...
	ok_or_else<T, E>(this: Option<T>, err: () => E): Result<T, E> {
		return new resultImpl(types.ok_or_else<T, E>(this.value, err));
	}
