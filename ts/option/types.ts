...
export const _None = /*#__NO_SIDE_EFFECTS__*/ <T = never>(): Option<T> =>
	Left<null, T>(null);
...
export const Some = /*#__NO_SIDE_EFFECTS__*/ <T>(v: T): Option<T> =>
	Right<T, null>(v);

/** True if it’s Some. */
export const is_some = /*#__NO_SIDE_EFFECTS__*/ <T>(o: Option<T>) =>
	is_right(o);
...
export function unwrap_unchecked<T>(o: Option<T>): T {
	return either<null, T, T>(
		o,
		() => {
			throw new Error('unwrap_unchecked called on None');
		},
		v => v
	);
}
...
/*#__NO_SIDE_EFFECTS__*/
export function unwrap_or_else<T, T2>(
	o: Option<T>,
	fallback: () => T2
): T | T2 {
	return either<null, T, T | T2>(
...
export function flatten<T>(o: Option<Option<T>>): Option<T> {
	...
	return either<null, Option<T>, Option<T>>(
		o,
		() => None,
		x => x
	);
...
/*#__NO_SIDE_EFFECTS__*/
export function and_then_flatten<T, O>(
	o: Option<T>,
	f: (v: T) => Option<O>
): Option<O> {
	return either<null, T, Option<O>>(
...
export function ok_or<T, E>(self: Option<T>, err: E): Result<T, E> {
	return either<null, T, Result<T, E>>(
		self,
		() => Err<E, T>(err),
		v => Ok<T, E>(v)
	);
...
export function option_zipped<T1, T2, T3>(
	...
	return flatten(and_then(o1, v => and_then(o2, vv => f(v, vv))));
}
...
export function option_result_zipped<T1, T2, T3, E1, E2>(
	...
	return option_zipped(o1, o2, (a, b) =>
		result_zipped(a, b, (c, d) => f(c, d))
	);
}
...
export function option_result_transpose<T, E>(
	...
	return either<null, Result<T, E>, Result<Option<T>, E>>(
		o,
		() => Ok<Option<T>, E>(None),
...
export async function option_promise_transpose<T>(
	...
	return either<null, Promise<T>, Promise<Option<T>>>(
		o,
		async () => None,
...
export async function option_result_promise_transpose<T, E>(
	...
	return option_promise_transpose(
		and_then(o, v => result_promise_transpose(v))
	);
}

export function result_to_option<T>(r: Result<T, unknown>): Option<T> {
...
export function option_result_result_flatten<T, E1, E2>(
	...
	return and_then(r, v => result_flatten(v));
}
...
export function option_result_and_then_flatten<T, T2, E, E2>(
	...
	return option_result_result_flatten(option_result_and_then(r, f));
}
...
export function option_result_option_result_flatten<T, E1, E2>(
	...
	return result_option_transpose(
		result_and_then(
			option_result_transpose(
				and_then(
					option_result_and_then(o, v => option_result_transpose(v)),
					v => result_flatten(v)
				)
			),
			v => flatten(v)
...
export function option_from_maybe_undefined<T>(v: T | undefined): Option<T> {
