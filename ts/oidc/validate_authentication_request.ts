...
function spaceSeparatedArrayEqual(a: string, b: string) {
	const [as, bs] = [new Set(a.split(' ')), new Set(b.split(' '))];
...
export function validateAuthenticationRequest(
	...
	const supportedScopesArray = Array.isArray(config.scopes_supported)
	...
	const supportedResponseTypesArray = Array.isArray(
	...
	if (
		![...supportedResponseTypes].some(t =>
			spaceSeparatedArrayEqual(req.response_type, t)
		)
	) {
		return option.Some(
			new Error(`missing response type support: ${req.response_type}`)
		);
	}
