...
export const Date = memo(function(props: DateProps) {
	...
	const dateParts: ReactyDateTimeFormatPart[] = new Intl.DateTimeFormat(locale, { dateStyle: 'full' })
	...
	if (ordinalIndex != -1) switch (locale.language) {
		case "en":
			...
			dateParts[dateParts.findIndex(v => v.type === "weekday")]!.value += ",";
	...
	const toRender = dateParts.map(v => <Fragment key={v.type}>{v.value}</Fragment>);
